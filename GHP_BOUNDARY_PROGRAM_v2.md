...
### B.4 The Riemann adjacency map (recorded so the fence is precise)

Following the REINMANN refutation (A.3), the program's Riemann posture is here recorded positively as well as negatively, so future sessions cite the honest adjacencies instead of re-deriving or re-inflating them. The posture itself: **neither a proof nor a disproof of the Riemann Hypothesis would inform GHP.** RH's truth value is identical in a world where GHP is right and one where it is wrong, so by the discriminator criterion it cannot discriminate; the quarantine on φ–zeta claims stands untouched. What exists is an adjacency map, every entry external, adversarially verified 2026-07-29 (five independent referees), receipts in `experiments/riemann_adjacency_gate_2026-07-29.py`:

1. **Fractal strings (LIT-RH001).** The middle-thirds Cantor set — the canonical ternary geometry — has complex dimensions in a vertical arithmetic progression of period 2π/log 3 (machine-verified to 1e-14), and RH is equivalent to an inverse spectral problem for fractal strings solvable for every dimension in (0,1) except 1/2 (Lapidus–Maier 1995; Lapidus–van Frankenhuijsen). The one published home where "ternary geometry with loops" honestly meets the Hypothesis.

2. **The geometric route (LIT-RH002).** The RH analogue is *proven* for varieties over finite fields (Weil for curves; Deligne 1974 in general) via Frobenius eigenvalues on étale cohomology — the one world where "geometry is the way" is a theorem, and standardly cited as the strongest evidence for classical RH. The widely held — heuristic, **not established** — diagnosis of why the number-field case stays open: the analogous geometric object for Spec Z (e.g. the self-product analogue of C × C used in the curve proof) has never been constructed. Deninger's program (1998 ICM) seeks a foliated flow whose **closed orbits are the primes** (length log p) with the explicit formula as a Lefschetz trace formula; Connes–Consani (2009–2024) seek the missing base. Constructing the loop-geometry under the integers is the mainstream best guess at the road to RH, and it has resisted for decades. It is not proven that building it would suffice.

3. **Primes as knots (LIT-RH002).** Arithmetic topology (Mazur 1960s; Morishita, *Knots and Primes*, Springer 2012/2024): compactified Spec Z behaves like a closed 3-manifold (étale cohomological dimension 3), primes like knots inside it, with real dictionary entries — for p ≡ q ≡ 1 (mod 4), mod-2 linking number ↔ Legendre symbol, quadratic reciprocity as linking symmetry. Connes–Consani 2024 (arXiv:2401.08401) connects this to the adele class space. *Design-rhyme note:* the external Luminara portal (zeb23ediah/luminara-portal) renders its 27 glyphs as torus knots T(p,q) on a golden torus (R = φ), and its SPECTRUM room implements the explicit-formula rebuild (ψ staircase vs 120 computed zeros, pre-pinned tolerances) plus the GUE sine-kernel pair correlation. The glyph-knot / prime-knot resemblance is recorded as a **rhyme, derived from neither side**; the portal is an educational instrument, not evidence.

4. **The pulse (LIT-RH003).** The Hardy Z-function Z(t) = e^{iθ(t)}ζ(1/2 + it) is real-valued on the real axis with |Z(t)| = |ζ(1/2+it)|; critical-line zeros are exactly its real zeros, so every sign change certifies a zero — the rigorous form of "the zeta pulses one way then back and the truths are the crossings." Caveat carried verbatim from review: equating *all* zeta zeros with the crossings of this oscillating function is equivalent to RH **plus** odd multiplicity — the pulse picture presumes the Hypothesis rather than suggesting it.

5. **The 1–4–27–108 chain (LIT-RH003).** H(3) = 1¹·2²·3³ = 108; the hyperfactorial's growth constant is the Glaisher–Kinkelin A, with the exact identity ln A = 1/12 − ζ′(−1) (machine-verified to 8e-8 from the raw asymptotic). The sequence 1, 4, 27 does reach a genuine zeta-derivative value — as a property of the sequence's asymptotics, **not of the number 108**, and it touches ζ′(−1), not the zeros.

Standing rule, restated: every entry above is external mathematics or an external instrument, citable as analogue or prior-art context only. None is GHP evidence under any outcome; any new φ–zeta literature item remains fringe-by-default under the A.3 reopen bar.
