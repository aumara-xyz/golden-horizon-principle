...
- ledger_anchor: CALIBRATION for GH-RECOV (ledger row GH-RECOV; master AH.4 Priority 1). This is NOT a new GHP discriminator. It runs the KAM/anti-resonance MECHANISM that GH-RECOV's recovery task was a PROXY for, in that mechanism's native habitat (the Chirikov standard map), to reconcile GH-RECOV's silver-win against the textbook expectation that golden is the last torus to break. Related: M-005 (phi-content is extremality-only), GOLDEN-HEAL-v1/v2 prereg contracts.
