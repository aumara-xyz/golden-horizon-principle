#!/usr/bin/env python3
"""v19c null controls for model-revision memory."""
...
OUT = ROOT / "golden_zipper_v19c_outputs"
...
def surrogate_score(seq: np.ndarray, kind: str) -> float:
    if kind == "density":
        surrogate = density_random_surrogate(seq)
    elif kind == "block":
        surrogate = block_shuffle_surrogate(seq)
    elif kind == "markov":
        surrogate = markov_surrogate(seq)
    else:
    ...
    return composite_score(evaluate_model(surrogate, diag))
...
def plot_mode_summary(rows: list[dict], path: Path) -> None:
    ...
    markov = [row["mean_markov_gap"] for row in rows]
    x = np.arange(len(labels))
    width = 0.35
    plt.figure(figsize=(10, 5))
    plt.bar(x - width / 2, comps, width=width, label="mean composite")
    plt.bar(x + width / 2, markov, width=width, label="markov gap")
...
def main() -> None:
    ...
    modes = ["real", "time_reversed", "random_projection", "phase_scrambled"]
    ...
    for mode in modes:
        ...
        seq, _ = build_sequence(best["alpha"], cond, mode)
        ...
        mode_rows.append(
            {
                ...
                "mean_density_gap": mean_composite - surrogate_score(seq, "density"),
                "mean_block_gap": mean_composite - surrogate_score(seq, "block"),
                "mean_markov_gap": mean_composite - surrogate_score(seq, "markov"),
            }
        )
    ...
    report = f"""# Golden Zipper v19c - Model Revision Null Controls
...
- real density gap: `{real['mean_density_gap']:.3f}`
- real block gap: `{real['mean_block_gap']:.3f}`
- real Markov gap: `{real['mean_markov_gap']:.3f}`

Strongest null:
- `{strongest_null['mode']}` mean composite: `{strongest_null['mean_composite']:.3f}`

Interpretation:
- This panel asks whether the model-revision signal survives time reversal, random projection, phase scrambling, and Markov/density/block surrogates.
- A strong result requires real sequences to beat null modes and keep positive surrogate gaps.
...
"""
    ...
    print(f"real density gap: {real['mean_density_gap']:.3f}")
    print(f"real block gap: {real['mean_block_gap']:.3f}")
    print(f"real Markov gap: {real['mean_markov_gap']:.3f}")
