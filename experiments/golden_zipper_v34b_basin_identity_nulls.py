#!/usr/bin/env python3
"""v34b basin identity nulls.

This attacks the best v34 basin-recall object:
- real basin recall
...
OUT = ROOT / "golden_zipper_v34b_outputs"
V34_SUMMARY = ROOT / "golden_zipper_v34_outputs" / "summary.csv"

SEED = 20260523
RNG = np.random.default_rng(SEED)
MODES = ["real", "shuffled_similarity", "random_tint", "basin_break"]
...
def basin_weights_mode(slot: int, slot_count: int, recall_condition, mode: str):
    ...
    broken_weights = [weight for _, weight in shuffled]
    return list(zip(members, broken_weights))
...
def main() -> None:
        ...
        summary_rows.append(
            {
                ...
                "basin_break_gap": real["score"] - next(row for row in subset if row["mode"] == "basin_break")["score"],
            }
        )
    ...
    report = f"""# Golden Zipper v34b - Basin Identity Nulls
...
- basin-break gap: `{golden['basin_break_gap']:.3f}`

Interpretation:
- This panel attacks the basin-recall object with basin-coupling nulls.
- A useful result keeps positive gaps when basin coherence is scrambled or broken.
"""
    ...
    print(f"golden basin-break gap: {golden['basin_break_gap']:.3f}")
