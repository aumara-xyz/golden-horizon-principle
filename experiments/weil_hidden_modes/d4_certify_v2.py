...
L=arb('7/10'); T=120; NE=int(sys.argv[1]) if len(sys.argv)>1 else 80; ns=[2*m for m in range(NE)]; ncut=ns[-1]+2
K=int(sys.argv[2]) if len(sys.argv)>2 else 48      # GL nodes per unit panel
...
def jn_all(nmax,z):
    ...
    return out
cn_pref={n:((2*n+1)/(2*L)).sqrt()*L*2/(2*pi).sqrt()*(-1)**(n//2) for n in ns}
...
eig_cert={"lambda_min_lower_bound":lam_low.str(12),"gershgorin_min":gmin.str(12),"lambda_max_VtV_upper":vtvmax.str(20),
...
out={"run":4,"NE":NE,"ncut":ncut,"prec_bits":192,"GL_nodes_per_unit_panel":K,"rho":2,"cover_boxes":J,"beta_star":beta.str(20),
...
json.dump(out,open(f"d4_results_NE{NE}_run4.json","w"),indent=1); print(json.dumps(out,indent=1))
