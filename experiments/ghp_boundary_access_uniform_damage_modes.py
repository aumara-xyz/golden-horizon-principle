#!/usr/bin/env python3
"""Uniform-smear crossover split by explicit damage mode.
...
OUT = ROOT / "ghp_boundary_access_uniform_damage_modes_outputs"
...
def main() -> None:
    ...
    rows = [
        evaluate_family_damage(family_name, noise_level, damage_mode, words, vocab_index)
        for noise_level in NOISE_LEVELS
        for damage_mode in DAMAGE_MODES
        for family_name in FAMILIES
    ]
    write_csv(rows, OUT / "uniform_damage_mode_metrics.csv")

    lines = [
        "# Boundary Access Uniform Damage Modes",
        "",
        f"- access cost `{ACCESS_COST}`",
        f"- switch cost `{SWITCH_COST}`",
        "",
        "Winner by noise, damage mode, and score view:",
    ]
    for noise_level in NOISE_LEVELS:
        for damage_mode in DAMAGE_MODES:
            subset = [
                row for row in rows
                if float(row["noise_level"]) == noise_level and row["damage_mode"] == damage_mode
            ]
            bits = []
            ...
            lines.append(f"- noise `{noise_level}` / {damage_mode}: " + ", ".join(bits))
