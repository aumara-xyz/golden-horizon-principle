#!/usr/bin/env python3
"""v58 core-frame recall.

Core memory stays mostly fixed; only the surrounding frame gets recolored.
...
OUT = ROOT / "golden_zipper_v58_outputs"
V41_SUMMARY = ROOT / "golden_zipper_v41_outputs" / "summary.csv"

SEED = 20260526
RNG = np.random.default_rng(SEED)
MODES = {
    "rigid_core": {"frame_gain": 0.00, "core_anchor": 0.18, "random_frame": False},
    "touchup_frame": {"frame_gain": 0.05, "core_anchor": 0.20, "random_frame": False},
    "balanced_frame": {"frame_gain": 0.12, "core_anchor": 0.22, "random_frame": False},
    "overwash_frame": {"frame_gain": 0.24, "core_anchor": 0.10, "random_frame": False},
}
...
def evaluate_recall(seq: np.ndarray, diag: dict[str, float]) -> dict[str, float]:
    ...
    metrics["frame_shift_rate"] = diag["frame_shift_events"] / max(diag["recall_hits"], 1.0)
    return metrics


def run_sequence(alpha: float, base_condition, recall_condition, frame_gain: float, core_anchor: float, random_frame: bool):
    ...
    split_events = context_distortions = blocked_contexts = overlap_events = phase_bind_events = macro_window_events = admission_band_events = frame_shift_events = 0
    ...
    for idx in range(base_condition.delay + 2, v41.v31b.v31.LENGTH):
        ...
        if field_recent:
            ...
            band_shape = np.exp(-0.5 * ((prediction_error - recall_condition.admission_center) / max(recall_condition.admission_width, 1e-9)) ** 2)
            ...
            if soft_admit >= soft_threshold:
                ...
                charges += recall_condition.recall_gain * 0.25 * field * (
                    field_charge + incoming_groove + support + recall_condition.phase_bind_gain * phase_bind_strength + recall_condition.window_blend_gain * field_layer + band_support
                )
                charges[slot] += recall_condition.recall_gain * 0.60 * (
                    charges[slot] + incoming_groove + 0.40 * field_layer + support + recall_condition.phase_bind_gain * phase_bind_strength + recall_condition.window_blend_gain * field_layer + band_support - competition
                )

                if frame_gain > 0.0:
                    frame_field = field.copy()
                    frame_field[slot] = 0.0
                    if frame_field.sum() > 0:
                        frame_field /= frame_field.sum()
                        if random_frame:
                            frame_field = frame_field[perm]
                        frame_push = frame_gain * (0.35 + field_layer + 0.5 * phase_bind_strength)
                        charges += 0.40 * frame_push * frame_field
                        charges[slot] += core_anchor * frame_push
                        layer_trace[:, 0] += 0.30 * frame_push * frame_field
                        if frame_push >= 0.04:
                            frame_shift_events += 1

                spill = recall_condition.context_pressure * max(context_similarity - recall_condition.similarity_threshold, 0.0) * max(charges[slot], 0.10) + competition * 0.5
                if spill > 0.0:
                ...
                blocked_contexts += 1
                charges[slot] += recall_condition.recall_gain * (0.08 + 0.08 * min(field_layer, 1.0) + 0.06 * min(phase_bind_strength, 1.0) + 0.04 * band_support)
    ...
    diag = {
        ...
        "frame_shift_events": float(frame_shift_events),
    }
    return seq, diag
...
def main() -> None:
    ...
    for flow in sorted({row["flow"] for row in rows}):
        ...
        bal=next(row for row in subset if row["mode"]=="balanced_frame")
        summary.append({
            "flow": flow,
            "best_mode": max(subset, key=lambda row: row["score"])["mode"],
            "balanced_score": bal["score"],
            "same_field_recall_rate": bal["same_field_recall_rate"],
            "same_core_recall_rate": bal["same_core_recall_rate"],
            "frame_shift_rate": bal["frame_shift_rate"],
            "balanced_vs_rigid_gap": bal["score"] - next(row for row in subset if row["mode"]=="rigid_core")["score"],
            "balanced_vs_touchup_gap": bal["score"] - next(row for row in subset if row["mode"]=="touchup_frame")["score"],
            "balanced_vs_overwash_gap": bal["score"] - next(row for row in subset if row["mode"]=="overwash_frame")["score"],
        })
    write_csv(summary, OUT / "summary.csv")
    ranked=sorted(summary, key=lambda row: row["balanced_score"], reverse=True)
    best=ranked[0]
    golden=next(row for row in summary if row["flow"]=="golden")
    write_text(OUT / "report.md", f"""# Golden Zipper v58 - Core Frame Recall

Best flow by balanced-frame score: `{best['flow']}` with `{best['balanced_score']:.3f}`
...
""")
    ...
    print(f"best flow by balanced-frame score: {best['flow']} {best['balanced_score']:.3f}")
    ...
    print(f"golden balanced vs touchup gap: {golden['balanced_vs_touchup_gap']:.3f}")
