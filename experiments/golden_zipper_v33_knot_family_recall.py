#!/usr/bin/env python3
"""v33 knot-family recall.

This tests whether a memory lives in a small knot-family rather than one exact slot:
- recall can return to the same local family and still count as the same memory
- context can tint the family without forcing exact-slot return
- breaking one exact slot should matter less if the family remains coherent
...
OUT = ROOT / "golden_zipper_v33_outputs"
...
def build_recall_conditions() -> list[RecallCondition]:
    full = [
        ...
        for family_radius in FAMILY_RADII
    ]
...
def score_recall(metrics: dict[str, float]) -> float:
    deepen = min(metrics["same_family_recall_rate"] / 0.50, 1.0)
...
def family_slots(slot: int, slot_count: int, radius: int) -> list[int]:
    return sorted({(slot + offset) % slot_count for offset in range(-radius, radius + 1)})


def build_knot_family_sequence(
    alpha: float,
    base_condition,
    ...
    for idx in range(base_condition.delay + 2, v31b.v31.LENGTH):
        ...
        family = family_slots(slot, base_condition.slot_count, recall_condition.family_radius)
...
def evaluate_recall(seq: np.ndarray, diag: dict[str, float]) -> dict[str, float]:
    ...
    return {
        ...
        "same_family_recall_rate": diag["same_family_recalls"] / recall_hits,
...
def main() -> None:
    ...
    for spec in specs:
        for recall_condition in recall_conditions:
            seq, diag = build_knot_family_sequence(spec.alpha, spec.condition, recall_condition)
            ...
            direct_rows.append(
                {
                    ...
                    "family_radius": recall_condition.family_radius,
                    **metrics,
    ...
    for spec in specs:
        ...
        recall_condition = RecallCondition(
            ...
            family_radius=int(best["family_radius"]),
        )
        seq, diag = build_knot_family_sequence(spec.alpha, spec.condition, recall_condition)
        ...
        summary_rows.append(
            {
                ...
                "best_family_radius": recall_condition.family_radius,
                ...
                "mean_same_family_recall_rate": float(np.mean([row["same_family_recall_rate"] for row in subset])),
    ...
    report = f"""# Golden Zipper v33 - Knot-Family Recall
...
- best family radius: `{golden['best_family_radius']}`
...
- mean same-family recall rate: `{golden['mean_same_family_recall_rate']:.3f}`
...
Interpretation:
- This panel tests whether a memory can live in a small family of nearby slots instead of one exact slot.
- A useful result should raise same-family recall without needing exact-slot return.
"""
    ...
    print(f"golden best family_radius: {golden['best_family_radius']}")
    print(f"golden recall survival rate: {golden['mean_recall_survival_rate']:.3f}")
    print(f"golden same-family recall rate: {golden['mean_same_family_recall_rate']:.3f}")
