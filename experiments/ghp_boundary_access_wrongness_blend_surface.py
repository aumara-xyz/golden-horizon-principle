#!/usr/bin/env python3
"""Blend surface between internal-scrambled and coherent-foreign wrongness.
...
OUT = ROOT / "ghp_boundary_access_wrongness_blend_surface_outputs"

NOISE_LEVELS = [0.20, 0.30, 0.40]
BLEND_LEVELS = [0.0, 0.25, 0.50, 0.75, 1.0]
...
def blended_wrong_signal(
    readable_a: np.ndarray,
    cross_family_truth: np.ndarray,
    blend: float,
) -> np.ndarray:
    permuted = readable_a[event.loop.RNG.permutation(len(readable_a))]
    mixed = (1.0 - blend) * permuted + blend * cross_family_truth
    return event.loop.normalize(mixed)


def evaluate_family_blend(
    family_name: str,
    noise_level: float,
    blend: float,
    words: dict[str, str],
    ...
    for _ in range(event.loop.TRIALS):
        ...
        for step in range(event.loop.TIMESTEPS - 1):
            ...
            damaged = blended_wrong_signal(readable_a, cross_truth, blend)
...
def main() -> None:
    ...
    rows = [
        evaluate_family_blend(family_name, noise_level, blend, words, vocab_index)
        for noise_level in NOISE_LEVELS
        for blend in BLEND_LEVELS
        for family_name in FAMILIES
    ]
    write_csv(rows, OUT / "wrongness_blend_metrics.csv")

    lines = [
        "# Boundary Access Wrongness Blend Surface",
        "",
        f"- access cost `{ACCESS_COST}`",
        f"- switch cost `{SWITCH_COST}`",
        "",
        "Winner by noise, blend level, and score view:",
    ]
    for noise_level in NOISE_LEVELS:
        for blend in BLEND_LEVELS:
            subset = [
                row for row in rows
                if float(row["noise_level"]) == noise_level and float(row["blend_level"]) == blend
            ]
            bits = []
            ...
            lines.append(f"- noise `{noise_level}` / blend `{blend}`: " + ", ".join(bits))
