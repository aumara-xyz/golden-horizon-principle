#!/usr/bin/env python3
"""v35b soft temporal echo.

This tests whether memory identity is helped by temporal echo:
- recall can move within a local region
- temporal layering acts like a soft coherence echo instead of a hard gate
- the region remains primary, while timing helps the region hold together
...
OUT = ROOT / "golden_zipper_v35b_outputs"
...
LAYER_MATCH_THRESHOLDS = [0.08, 0.16, 0.24]
BASIN_RADII = [2]
...
def score_recall(metrics: dict[str, float]) -> float:
    same_layer = min(metrics["same_layer_recall_rate"] / 0.35, 1.0)
        ...
        if recent_region:
            ...
            if soft_admit >= soft_threshold:
                ...
                if slot in recent_region:
                    same_slot_recalls += 1
                if slot_layer_strength >= recall_condition.layer_match_threshold:
                ...
                for member in recent_region:
                    charges[member] += recall_condition.recall_gain * 0.35 * (
                        region_charge + incoming_groove + 0.5 * region_layer_strength
                    )
                ...
                charges[slot] += recall_condition.recall_gain * 0.65 * (
                    charges[slot] + incoming_groove + 0.6 * slot_layer_strength
                )
                ...
                blocked_contexts += 1
                charges[slot] += recall_condition.recall_gain * (0.10 + 0.15 * min(layer_match, 1.0))

        layer_support = 0.30 * region_layer_strength
...
def main() -> None:
    ...
    report = f"""# Golden Zipper v35b - Soft Temporal Echo
...
Interpretation:
- This panel tests whether timing helps memory as a soft echo rather than a hard gate.
- A useful result should recover recall survival while keeping some same-layer signal alive.
"""
