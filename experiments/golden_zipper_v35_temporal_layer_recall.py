#!/usr/bin/env python3
"""v35 temporal layer recall.

This tests whether memory identity is preserved by temporal layering:
- recall can move within a local region
- but it counts as the same memory only when the timing stack still matches
- temporal layering may matter more than exact geometric shape
...
OUT = ROOT / "golden_zipper_v35_outputs"
...
LAYER_WINDOWS = [8, 13, 21]
RECENCY_DECAYS = [0.55, 0.72]
RESONANCE_GAINS = [0.08, 0.14]
LAYER_MATCH_THRESHOLDS = [0.18, 0.30, 0.42]
BASIN_RADII = [2]
...
def build_recall_conditions() -> list[RecallCondition]:
    full = [
        RecallCondition(
            ...
            layer_match_threshold=layer_match_threshold,
        ...
        for layer_match_threshold in LAYER_MATCH_THRESHOLDS
...
def basin_members(slot: int, slot_count: int, radius: int) -> list[int]:
...
def score_recall(metrics: dict[str, float]) -> float:
    same_layer = min(metrics["same_layer_recall_rate"] / 0.50, 1.0)
    same_region = min(metrics["same_region_recall_rate"] / 0.50, 1.0)
    exact = min(metrics["same_slot_recall_rate"] / 0.35, 1.0)
    ...
    return float(
        ...
        + 0.08 * distortion
        + 0.07 * blocked
        + 0.02 * (1.0 - metrics["rupture_rate"])
        + 0.01 * (1.0 - metrics["pollution"])
    )


def build_temporal_layer_sequence(
    alpha: float,
    base_condition,
    ...
    recall_survivals = 0
    same_slot_recalls = 0
    same_region_recalls = 0
    same_layer_recalls = 0
    split_events = 0
    ...
    for idx in range(base_condition.delay + 2, v31b.v31.LENGTH):
        ...
        region = basin_members(slot, base_condition.slot_count, recall_condition.basin_radius)
        ...
        slot_layer_strength = float(np.dot(layer_trace[slot], kernel))
        ...
        if recent_region:
            ...
            layer_match = min(region_layer_strength / max(recall_condition.resonance_gain, 1e-9), 1.5)
            if context_similarity >= recall_condition.similarity_threshold and layer_match >= recall_condition.layer_match_threshold:
                ...
                if slot in recent_region:
                    same_slot_recalls += 1
                if slot_layer_strength >= recall_condition.layer_match_threshold * 0.75:
                    same_layer_recalls += 1

                region_charge = np.mean([charges[member] for member in recent_region])
                for member in recent_region:
                    charges[member] += recall_condition.recall_gain * 0.35 * (region_charge + incoming_groove + region_layer_strength)
                    grooves[slot, member] += recall_condition.resonance_gain * (contact + phase_alignment)
                charges[slot] += recall_condition.recall_gain * 0.65 * (charges[slot] + incoming_groove + slot_layer_strength)
                layer_trace[slot, 0] += recall_condition.resonance_gain * (contact + phase_alignment)
                ...
                neighbor_bleed = recall_condition.context_pressure * tint * max(charges[slot], 0.10)
                ...
                blocked_contexts += 1
                charges[slot] += recall_condition.recall_gain * 0.10

        layer_support = 0.5 * region_layer_strength
        charges[slot] += base_condition.stack_gain * contact + neighbor_support + phase_support + layer_support
        effective_charge = charges[slot] + incoming_groove + neighbor_support + phase_support + layer_support
    ...
    return seq, {
        ...
        "same_layer_recalls": float(same_layer_recalls),
...
def evaluate_recall(seq: np.ndarray, diag: dict[str, float]) -> dict[str, float]:
    ...
    return {
        ...
        "same_layer_recall_rate": diag["same_layer_recalls"] / recall_hits,
...
def plot_summary(rows: list[dict], path: Path) -> None:
    ...
    layer = [row["mean_same_layer_recall_rate"] for row in rows]
    survival = [row["mean_recall_survival_rate"] for row in rows]
    x = np.arange(len(labels))
    width = 0.25
    plt.figure(figsize=(11, 5))
    plt.bar(x - width, score, width=width, label="mean score")
    plt.bar(x, layer, width=width, label="same-layer recall")
    plt.bar(x + width, survival, width=width, label="recall survival")
...
def main() -> None:
    ...
    for spec in specs:
        for recall_condition in recall_conditions:
            seq, diag = build_temporal_layer_sequence(spec.alpha, spec.condition, recall_condition)
            ...
            direct_rows.append(
                {
                    ...
                    "layer_match_threshold": recall_condition.layer_match_threshold,
    ...
    for spec in specs:
        ...
        recall_condition = RecallCondition(
            ...
            layer_match_threshold=best["layer_match_threshold"],
        ...
        summary_rows.append(
            {
                ...
                "best_layer_window": recall_condition.layer_window,
                "best_recency_decay": recall_condition.recency_decay,
                "best_resonance_gain": recall_condition.resonance_gain,
                "best_layer_match_threshold": recall_condition.layer_match_threshold,
                "best_basin_radius": recall_condition.basin_radius,
                ...
                "mean_same_layer_recall_rate": float(np.mean([row["same_layer_recall_rate"] for row in subset])),
    ...
    report = f"""# Golden Zipper v35 - Temporal Layer Recall
...
- best layer window: `{golden['best_layer_window']}`
- best recency decay: `{golden['best_recency_decay']:.3f}`
- best resonance gain: `{golden['best_resonance_gain']:.3f}`
- best layer match threshold: `{golden['best_layer_match_threshold']:.3f}`
...
- mean same-layer recall rate: `{golden['mean_same_layer_recall_rate']:.3f}`
...
Interpretation:
- This panel tests whether memory identity is preserved more by temporal layering than by exact geometric location.
- A useful result should keep same-layer recall high even when same-slot recall stays modest.
"""
    ...
    print(f"golden best layer_window: {golden['best_layer_window']}")
    print(f"golden best recency_decay: {golden['best_recency_decay']:.3f}")
    print(f"golden same-layer recall rate: {golden['mean_same_layer_recall_rate']:.3f}")
