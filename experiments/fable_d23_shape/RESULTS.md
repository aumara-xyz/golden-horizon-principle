...
## Correction (appended 2026-09-14; nothing above edited)
The profile fit in §A used the D22 floors that D24 marked VOID as certified endpoints and that are, at L = 0.4–0.6, loose by 14–20 % because invisible shifts were kept (see the D22 correction). The slopes 53/65/83 and the e^{2L} fit are therefore UNVERIFIED until recomputed on visible-shift, repaired certificates; the qualitative statement (slopes increase with L) is expected to survive but is not established. The prolate overlaps in §B used the corresponding minimizers; D24 §5 is right that a shape overlap is not an energy identity — the overlap is a MEASURED (floating) observation about those particular finite minimizers, and the "λmin ≈ min_Ω W(ψ₀^Ω)" statement is a proposal, not a result. The D23 generating script was not committed with the round (provenance gap noted by D24); it is the inline script recorded in d23_results.log's header and is to be added as d23.py.
