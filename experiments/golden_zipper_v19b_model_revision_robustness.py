#!/usr/bin/env python3
"""v19b robustness/null panel for model-revision memory."""
...
OUT = ROOT / "golden_zipper_v19b_outputs"
...
def build_anchors() -> list[AnchorSpec]:
    anchors = [
        ...
        AnchorSpec("fib_8_13", 8.0 / 13.0, "rational"),
        ...
        AnchorSpec("noble_2", continued_fraction_value([4, 1, 5], tail_ones=18), "noble"),
        AnchorSpec("bounded_cf_1", continued_fraction_value([1, 2, 1, 2, 1, 2], tail_ones=14), "bounded_cf"),
    ...
    for idx in range(6):
        coeffs = list(RNG.integers(1, 9, size=6))
        anchors.append(AnchorSpec(f"random_cf_{idx+1}", continued_fraction_value(coeffs, tail_ones=8), "random_cf"))
    return anchors
    ...
    for idx, value in enumerate(intensity):
        ...
        if model == "surprise_only":
            ...
            if revision_ok or intense_revision:
                ...
                rejected += 1
        elif model == "model_revision_witness":
            revision_ok = revision_size >= condition.revision_threshold and coherence_cost <= condition.coherence_limit
            intense_revision = surprise >= condition.surprise_high and coherence_cost <= condition.coherence_limit
            if revision_ok or intense_revision:
                admitted += 1
...
def plot_model_summary(rows: list[dict], path: Path) -> None:
    ...
    density_gaps = [row["mean_density_gap"] for row in rows]
    block_gaps = [row["mean_block_gap"] for row in rows]
    x = np.arange(len(labels))
    width = 0.25
    plt.figure(figsize=(10, 5))
    plt.bar(x - width, comps, width=width, label="mean composite")
    plt.bar(x, density_gaps, width=width, label="density gap")
    plt.bar(x + width, block_gaps, width=width, label="block gap")
...
def main() -> None:
    ...
    for model in models:
        subset = [row for row in direct_rows if row["model"] == model]
        ...
        block = block_shuffle_surrogate(seq)
        zero_diag = {
            "admitted": float(np.sum(seq >= 0)),
            "rejected": 0.0,
            "revisions": 0.0,
            "writes": 0.0,
            "witnesses": 0.0,
        ...
        density_metrics = evaluate_model(density, {**zero_diag, "admitted": float(np.sum(density >= 0))})
        block_metrics = evaluate_model(block, {**zero_diag, "admitted": float(np.sum(block >= 0))})
        ...
        model_rows.append(
            {
                ...
                "mean_block_gap": mean_composite - composite_score(block_metrics),
            }
        )

    family_rows: list[dict] = []
    for family in sorted({row["family"] for row in direct_rows}):
        subset = [row for row in direct_rows if row["family"] == family and row["model"] == "model_revision_witness"]
        family_rows.append(
            {
                "family": family,
                "mean_composite": float(np.mean([row["composite_score"] for row in subset])),
    ...
    report = f"""# Golden Zipper v19b - Model Revision Robustness
...
- `model_revision_witness` density surrogate gap: `{witness['mean_density_gap']:.3f}`
- `model_revision_witness` block surrogate gap: `{witness['mean_block_gap']:.3f}`

Interpretation:
- This robustness panel widens anchors and threshold settings around the v19 model-revision rule.
- A durable positive would require model-revision variants to beat surprise-only and keep positive density/block surrogate gaps.
- This is still toy telemetry only and should not harden the master by itself.
"""
    ...
    print(f"model_revision_witness density gap: {witness['mean_density_gap']:.3f}")
    print(f"model_revision_witness block gap: {witness['mean_block_gap']:.3f}")
