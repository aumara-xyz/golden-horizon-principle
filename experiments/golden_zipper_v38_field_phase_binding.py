...
This tests whether memory identity depends on phase binding inside a field:
- memory is represented by a local relational field
- the field is stronger when its active parts stay phase-linked
- recall survives when field presence and binding both hold together
...
OUT = ROOT / "golden_zipper_v38_outputs"
...
def main() -> None:
    ...
    report = f"""# Golden Zipper v38 - Field Phase Binding
...
Interpretation:
- This panel tests whether a relational field needs phase binding to hold together as one memory.
- A useful result should improve field-level identity without forcing exact-core pinning.
"""
