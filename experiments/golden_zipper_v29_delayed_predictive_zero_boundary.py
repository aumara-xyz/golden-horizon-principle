#!/usr/bin/env python3
"""v29 delayed predictive zero-boundary panel.

This panel tests the Plinko-like intuition:
- hidden information happens first
- the observer receives it after a delay
- the observer predicts the current now
- memory writes occur when delayed input lands inside a prediction map
  and updates the model without rupturing it
...
OUT = ROOT / "golden_zipper_v29_outputs"
...
def build_sequence(alpha: float, condition: Condition) -> tuple[np.ndarray, dict[str, float]]:
    ...
    for idx in range(condition.delay + 2, LENGTH):
        ...
        predicted_now = prediction + condition.delay * velocity

        fit_error = abs(delayed - predicted_now)
        revision = condition.prediction_gain * (delayed - predicted_now)
        revision_size = abs(revision)
        fit_errors.append(fit_error)
        revision_sizes.append(revision_size)

        near_zero = abs(delayed) <= condition.window_width
        fits_map = fit_error <= condition.fit_limit
        can_revise = revision_size >= condition.revision_threshold
        rupture = fit_error >= condition.rupture_limit

        pocket *= condition.pocket_decay
        if pocket_open:
            pocket_age += 1

        if near_zero and fits_map:
            contact = (1.0 - abs(delayed) / max(condition.window_width, 1e-9)) * (1.0 - fit_error / max(condition.fit_limit, 1e-9))
            pocket += condition.pocket_gain * max(contact, 0.0)
            if not pocket_open:
                seq[idx] = 0
                witnesses += 1
                pocket_open = True
        ...
        if rupture and near_zero:
            ...
            pocket_open = False
            pocket_age = 0
            prediction = 0.98 * prediction + 0.02 * delayed
            velocity = 0.98 * velocity + 0.02 * (delayed - previous_delayed)
            continue
        ...
        if pocket_open and pocket >= condition.write_threshold and can_revise and fits_map:
            ...
            pocket_open = False
            pocket_age = 0
            prediction = predicted_now + revision
            velocity = (1.0 - condition.prediction_gain) * velocity + condition.prediction_gain * (delayed - previous_delayed)
            continue

        if pocket_open and pocket < 0.06 and not near_zero:
            seq[idx] = -1
            releases += 1
            pocket_ages.append(pocket_age)
            pocket = 0.0
        ...
        if fits_map:
            prediction = predicted_now + 0.35 * revision
            velocity = (1.0 - 0.35 * condition.prediction_gain) * velocity + 0.35 * condition.prediction_gain * (delayed - previous_delayed)
        else:
            prediction = 0.998 * prediction + 0.002 * delayed
            velocity = 0.998 * velocity + 0.002 * (delayed - previous_delayed)
...
def score(metrics: dict[str, float]) -> float:
    fit_term = 1.0 - min(metrics["mean_fit_error"] / 0.65, 1.0)
    revision_term = min(metrics["mean_revision_size"] / 0.08, 1.0)
...
def main() -> None:
    ...
    report = f"""# Golden Zipper v29 - Delayed Predictive Zero Boundary
...
Interpretation:
- This panel treats experienced now as a delayed predictive render.
- A useful result would show witness-to-write conversion, low rupture, and positive null gaps.
- Phi/golden is still treated as an anchor candidate, not as the write point.
"""
