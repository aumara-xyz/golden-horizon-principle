#!/usr/bin/env python3
"""Uniform-smear crossover test across rescue policies.
...
OUT = ROOT / "ghp_boundary_access_uniform_policy_surface_outputs"

NOISE_LEVELS = [0.20, 0.30, 0.40]
FAMILIES = ["fibonacci", "generic_ternary"]
POLICIES = ["always_fresh", "always_deep", "always_layered", "adaptive_context"]
ACCESS_COST = 0.02
SWITCH_COST = 0.03
...
def payload_family(
    ...
    if policy_name == "adaptive_context":
        return context_policy.choose_family(policy_name, damage_mode, "noisy")
...
def evaluate_family_policy(
    ...
    for _ in range(event.loop.TRIALS):
        ...
        for step in range(event.loop.TIMESTEPS - 1):
            ...
            penalty = SWITCH_COST if prev_policy_family is not None and family != prev_policy_family else 0.0
            prev_policy_family = family
...
def main() -> None:
    ...
    rows = [
        evaluate_family_policy(family_name, noise_level, policy_name, words, vocab_index)
        for noise_level in NOISE_LEVELS
        for policy_name in POLICIES
        for family_name in FAMILIES
    ]
    write_csv(rows, OUT / "uniform_policy_surface.csv")

    lines = [
        "# Boundary Access Uniform Policy Surface",
        "",
        f"- access cost `{ACCESS_COST}`",
        f"- switch cost `{SWITCH_COST}`",
        "",
        "Winner by noise level, policy, and score view:",
    ]
    for noise_level in NOISE_LEVELS:
        for policy_name in POLICIES:
            subset = [
                row for row in rows
                if float(row["noise_level"]) == noise_level and row["policy"] == policy_name
            ]
            bits = []
            ...
            lines.append(f"- noise `{noise_level}` / {policy_name}: " + ", ".join(bits))
