#!/usr/bin/env python3
"""Rank-matched coherent control for Boundary Access repair features.
...
OUT = ROOT / "ghp_boundary_access_rank_matched_control_outputs"

VARIANTS = ["rolled", "reversed", "permuted", "cross_rankmatched"]
COHERENT_VARIANTS = ["cross_rankmatched"]
PACKS = chunk_control.PACKS
...
def rank_match(source_shape: np.ndarray, template_values: np.ndarray) -> np.ndarray:
    order = np.argsort(source_shape)
    values = np.sort(template_values)
    out = np.zeros_like(source_shape, dtype=float)
    out[order] = values
    return generalization.event.loop.normalize(out)
                    ...
                    for _ in range(generalization.TRIALS_PER_SCENARIO):
                        ...
                        for step in range(event.loop.TIMESTEPS - 1):
                            ...
                            if variant == "cross_rankmatched":
                                damaged = rank_match(cross_family_b, readable_a)
                            else:
                            ...
                            belief_features = chunk_control.inertia_switcher.derive_axes(damaged, helper, wake, deep_trace)
                            rows.append(
                                {
                                    "seed": seed,
...
def evaluate_pack(
    ...
    model = chunk_control.fit_pack(train_rows, pack)
    chosen = [chunk_control.choose_family(row, pack, model) for row in test_rows]
    overall = float(np.mean([choice == row["target_family"] for choice, row in zip(chosen, test_rows)]))
    summary = {"pack": pack, "dimensions": int(len(chunk_control.features_for_pack(test_rows[0], pack))), "overall_accuracy": overall}
    ...
    for scenario in sorted({str(row["scenario"]) for row in test_rows}):
        ...
        subset_chosen = [chunk_control.choose_family(row, pack, model) for row in subset]
...
def main() -> None:
    ...
    write_csv(summary_rows, OUT / "rank_matched_control_summary.csv")
    write_csv(scenario_rows, OUT / "rank_matched_control_scenarios.csv")

    best = summary_rows[0]
    target = [row for row in scenario_rows if row["pack"] == best["pack"]]
    lines = [
        "# Boundary Access Rank-Matched Control",
        "",
        "- question: does repair survive after coherent and internal variants are forced to share a ranked-value profile?",
