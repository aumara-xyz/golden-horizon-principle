...
## Correction (appended 2026-09-14 after Astra D24 and the Nebius lab's D25 Stage A; nothing above edited)
Two defects in the D22 certificates, both found by others: (1) d22_certify.py inherited d5_certify.py's hard-coded prime set {2, 3, 4}; at L = 0.4, 0.5, 0.6 some of these shifts do not fit the window (log 3 = 1.099 > 0.8; log 4 = 1.386 > 1.0 and > 1.2). Keeping them is VALID for W (their full-line correlations vanish) but makes the reduced form R_T strictly weaker than the visible-shift reduction (D24 §4.1 identity); the Nebius lab measured the floor lift at L = 0.4 odd: 0.0124019 → 0.0141716 (+14.3 %), and the D22 candidate builder (visible-only) had already shown the same gap (even L = 0.4: 1.72e-4 vs 1.449e-4). (2) D24 §3.2–3.3: the quadrature allowance used K nodes where the bound wants K + 1, and the scalar checker was applied unshifted; D24 supplies conservative repaired floors and a rational-arithmetic replacement checker. Status: the five D22 positivity statements STAND at D24's conservative constants; the twelve-digit endpoints printed above are VOID as certified endpoints; the certificates should be re-run with the visible-shift reduction and the repaired allowance before any profile is fitted to them.
