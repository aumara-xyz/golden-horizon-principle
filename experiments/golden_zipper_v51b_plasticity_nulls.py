#!/usr/bin/env python3
"""v51b plasticity nulls.

This attacks the plasticity rule:
- balanced plastic
- rigid
- overplastic
- random plastic
...
OUT = ROOT / "golden_zipper_v51b_outputs"
V41_SUMMARY = ROOT / "golden_zipper_v41_outputs" / "summary.csv"

MODES = {
    "balanced": dict(binding_mode="real", plasticity_gain=0.12, anchor_gain=0.14, random_plastic=False),
    "rigid": dict(binding_mode="real", plasticity_gain=0.00, anchor_gain=0.18, random_plastic=False),
    "overplastic": dict(binding_mode="real", plasticity_gain=0.24, anchor_gain=0.06, random_plastic=False),
    "random_plastic": dict(binding_mode="real", plasticity_gain=0.12, anchor_gain=0.14, random_plastic=True),
}
...
def main() -> None:
    ...
    for spec, recall_condition in specs:
        for mode, config in MODES.items():
            seq, diag = v51.run_sequence(spec.alpha, spec.condition, recall_condition, **config)
    ...
    for flow in sorted({row["flow"] for row in rows}):
        ...
        bal = next(row for row in subset if row["mode"] == "balanced")
        summary_rows.append(
            {
                "flow": flow,
                "balanced_score": bal["score"],
                "rewrite_drift_rate": bal["rewrite_drift_rate"],
                "same_field_recall_rate": bal["same_field_recall_rate"],
                "balanced_vs_rigid_gap": bal["score"] - next(row for row in subset if row["mode"] == "rigid")["score"],
                "balanced_vs_overplastic_gap": bal["score"] - next(row for row in subset if row["mode"] == "overplastic")["score"],
                "balanced_vs_random_gap": bal["score"] - next(row for row in subset if row["mode"] == "random_plastic")["score"],
            }
        )

    write_csv(summary_rows, OUT / "summary.csv")
    golden = next(row for row in summary_rows if row["flow"] == "golden")
    report = f"""# Golden Zipper v51b - Plasticity Nulls
