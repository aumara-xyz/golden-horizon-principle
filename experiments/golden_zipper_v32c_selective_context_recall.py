#!/usr/bin/env python3
"""v32c selective context recall.

This repairs v32b by making context effects selective:
- only similar context is allowed to bend the groove strongly
- dissimilar context should mostly fail to rewrite the memory
...
OUT = ROOT / "golden_zipper_v32c_outputs"

SEED = 20260523
RNG = np.random.default_rng(SEED)

CONTEXT_PRESSURES = [0.10, 0.22, 0.36]
SIMILARITY_THRESHOLDS = [0.55, 0.70, 0.82]
...
def score_recall(metrics: dict[str, float]) -> float:
    ...
    blocked = min(metrics["blocked_context_rate"] / 0.45, 1.0)
    return float(
...
def build_selective_context_sequence(
    alpha: float,
    base_condition,
        ...
        if idx - memory_age[slot] <= recall_condition.recall_window:
            ...
            if context_similarity >= recall_condition.similarity_threshold:
                ...
                if target_slot == slot:
                    ...
                    charges[slot] += recall_condition.recall_gain * 0.35
...
def main() -> None:
    ...
    for spec in specs:
        for recall_condition in recall_conditions:
            seq, diag = build_selective_context_sequence(spec.alpha, spec.condition, recall_condition)
    ...
    report = f"""# Golden Zipper v32c - Selective Context Recall
...
Interpretation:
- This panel asks whether only sufficiently similar present context should recolor a memory groove.
- A useful result would show stronger survival, lower distortion, and positive density gap compared with v32b.
"""
