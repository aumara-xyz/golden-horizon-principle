...
CERT = HERE / 'd26_cert_visible_odd_L0.4_T160_N160.json'

cert = json.loads(CERT.read_text())
lam0 = arb('0.0141715765223')          # certified eigenbound (printed value; radius negligible vs d)
eps_D = arb('1.412e-362')              # rounded UP from the printed interval
eps_C = arb('4.994e-178')
eps_p = arb('1.651e-988')
norm_pN = arb('0.10369343')            # upper-rounded
...
def run_T(T, K=64):
    arch, mass = compact_integral(b, 1, T, L, K=K)
...
(HERE / 'd26-bracket.json').write_text(json.dumps(out, indent=1) + '\n')
