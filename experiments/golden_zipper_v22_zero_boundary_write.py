#!/usr/bin/env python3
"""v22 zero-boundary write panel.

Toy telemetry only. Not physics evidence. Not proof of GHP.

This panel resets the toy around a zero-boundary observer:
- negative side: hidden / unresolved / not-yet-written
- zero: witness boundary
- positive side: admitted / written / stabilized

Phi-like flow is treated as a background motion candidate, not the write point.
"""
...
OUT = ROOT / "golden_zipper_v22_outputs"
...
BETAS = [0.0, 0.011]
DRIFTS = [0.0, 0.003]
WRITE_THRESHOLDS = [0.10, 0.18, 0.26]
STABILITY_THRESHOLDS = [0.05, 0.10, 0.16]
OVERLOAD_THRESHOLDS = [0.45, 0.65, 0.85]
MAX_CONDITIONS = 96
...
def build_hidden_trace(alpha: float, condition: Condition) -> np.ndarray:
    ...
    slow = 0.35 * np.sin(2.0 * math.pi * ((alpha / PHI) * n + 0.31))
    drift = condition.drift * (n / max(LENGTH - 1, 1))
    hidden = core + slow + drift
...
def build_sequence(alpha: float, condition: Condition) -> tuple[np.ndarray, dict[str, float]]:
    ...
    for idx in range(1, LENGTH):
        prev = hidden[idx - 1]
        cur = hidden[idx]
        crosses_zero = (prev < 0 <= cur) or (prev > 0 >= cur)
        slope = abs(cur - prev)
        local_stability = float(np.std(hidden[max(0, idx - 4) : min(LENGTH, idx + 5)]))
        quality = slope - local_stability

        if witness_open:
            witness_age += 1
            if witness_age > 6 and seq[idx - 1] == 0:
                seq[idx - 1] = -1
                release_total += 1
                witness_open = False
                witness_age = 0
        ...
        if slope >= condition.overload_threshold:
            seq[idx] = 2
            overload_total += 1
            witness_open = False
            witness_age = 0
        ...
        if local_stability <= condition.stability_threshold and slope >= condition.write_threshold:
            seq[idx] = 1
            stable_crossings += 1
            crossing_quality.append(quality)
            if witness_open:
                witness_to_write += 1
                witness_open = False
                witness_age = 0
...
def main() -> None:
    ...
    for flow in flows:
        ...
        density_gap = float(np.mean([row["composite_score"] for row in subset])) - composite_score(density_metrics)
        flow_rows.append(
            {
                "flow": flow.name,
                "family": flow.family,
    ...
    report = f"""# Golden Zipper v22 - Zero Boundary Write
...
Interpretation:
- This panel treats `0` as the observer boundary and tests whether hidden states cross into witness/write/release/overload more cleanly under some flows than others.
- Success here means the zero-boundary framing is a better memory toy than the prior direct revision/alignment toys.
"""
