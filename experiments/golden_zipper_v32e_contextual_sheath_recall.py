#!/usr/bin/env python3
"""v32e contextual sheath recall.

This keeps the memory in the same groove while allowing context to tint it:
- some context can recolor the groove
- unrelated context should mostly fail
- recall should survive without relocating the memory
...
OUT = ROOT / "golden_zipper_v32e_outputs"
...
def build_contextual_sheath_sequence(
    alpha: float,
    base_condition,
...
def main() -> None:
    ...
    for spec in specs:
        for recall_condition in recall_conditions:
            seq, diag = build_contextual_sheath_sequence(spec.alpha, spec.condition, recall_condition)
    ...
    report = f"""# Golden Zipper v32e - Contextual Sheath Recall
...
Interpretation:
- This panel tests whether context can tint a groove without relocating the memory.
- A useful result should recover same-slot recall while keeping distortion lower than the broad-context version.
"""
