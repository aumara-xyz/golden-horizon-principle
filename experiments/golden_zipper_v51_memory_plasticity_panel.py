#!/usr/bin/env python3
"""v51 memory plasticity panel.

This tests whether memory does best when recall can bend the field a little
...
OUT = ROOT / "golden_zipper_v51_outputs"
V41_SUMMARY = ROOT / "golden_zipper_v41_outputs" / "summary.csv"

SEED = 20260526
RNG = np.random.default_rng(SEED)
MODES = {
    "rigid": {"binding_mode": "real", "plasticity_gain": 0.00, "anchor_gain": 0.18, "random_plastic": False},
    "soft_plastic": {"binding_mode": "real", "plasticity_gain": 0.06, "anchor_gain": 0.18, "random_plastic": False},
    "balanced_plastic": {"binding_mode": "real", "plasticity_gain": 0.12, "anchor_gain": 0.14, "random_plastic": False},
    "overplastic": {"binding_mode": "real", "plasticity_gain": 0.24, "anchor_gain": 0.06, "random_plastic": False},
}
        ...
        if field_recent:
            ...
            if soft_admit >= soft_threshold:
                ...
                if plasticity_gain > 0.0:
                    ...
                    rewrite_push = plasticity_gain * (0.35 + field_layer + 0.5 * phase_bind_strength)
                    charges += 0.35 * rewrite_push * rewrite_field
                    charges[slot] += anchor_gain * rewrite_push
                    drift_slot = (slot + recall_condition.field_radius + 1) % base_condition.slot_count
                    drift_push = max(plasticity_gain - anchor_gain, 0.0) * rewrite_push
                    if drift_push > 0.0:
...
def main() -> None:
    ...
    for flow in sorted({row["flow"] for row in rows}):
        ...
        real = next(row for row in subset if row["mode"] == "balanced_plastic")
        summary_rows.append(
            {
                "flow": flow,
                "best_mode": max(subset, key=lambda row: row["score"])["mode"],
                "balanced_score": real["score"],
                ...
                "balanced_vs_rigid_gap": real["score"] - next(row for row in subset if row["mode"] == "rigid")["score"],
                "balanced_vs_soft_gap": real["score"] - next(row for row in subset if row["mode"] == "soft_plastic")["score"],
                "balanced_vs_overplastic_gap": real["score"] - next(row for row in subset if row["mode"] == "overplastic")["score"],
            }
        )
    ...
    report = f"""# Golden Zipper v51 - Memory Plasticity Panel

Toy telemetry only. Not physics evidence. Not proof of GHP.

Best flow by balanced-plastic score: `{best['flow']}` with `{best['balanced_score']:.3f}`
...
- balanced vs soft gap: `{golden['balanced_vs_soft_gap']:.3f}`
- balanced vs overplastic gap: `{golden['balanced_vs_overplastic_gap']:.3f}`

Interpretation:
- This panel asks whether memory should stay rigid, bend a little, or over-bend.
"""
    write_text(OUT / "report.md", report)

    print(f"files created: {OUT}")
    print(f"best flow by balanced-plastic score: {best['flow']} {best['balanced_score']:.3f}")
    ...
    print(f"golden balanced vs soft gap: {golden['balanced_vs_soft_gap']:.3f}")
    print(f"golden balanced vs overplastic gap: {golden['balanced_vs_overplastic_gap']:.3f}")
