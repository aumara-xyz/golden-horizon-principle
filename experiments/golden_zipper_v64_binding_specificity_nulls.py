#!/usr/bin/env python3
"""v64 binding-specificity nulls.

This attacks whether the binding relation itself matters:
- real field + binding + multiscale + admission band
- no binding
- shuffled phase
- mirrored phase
- flat binding
...
OUT = ROOT / "golden_zipper_v64_outputs"
V41_SUMMARY = ROOT / "golden_zipper_v41_outputs" / "summary.csv"

SEED = 20260526
RNG = np.random.default_rng(SEED)
MODES = ["real", "no_binding", "shuffled_phase", "mirrored_phase", "flat_binding", "no_multiscale", "no_band"]
...
def evaluate_recall(seq: np.ndarray, diag: dict[str, float]) -> dict[str, float]:
    return v41.evaluate_recall(seq, diag)
...
def run_sequence(alpha: float, base_condition, recall_condition, mode: str):
    ...
    window_levels = 1 if mode == "no_multiscale" else recall_condition.window_levels
    window_blend_gain = 0.0 if mode == "no_multiscale" else recall_condition.window_blend_gain
    admission_gain = 0.0 if mode == "no_band" else recall_condition.admission_gain
    ...
    for idx in range(base_condition.delay + 2, v41.v31b.v31.LENGTH):
        ...
        for neighbor in range(base_condition.slot_count):
            if field[neighbor] == 0.0:
                continue
            if mode == "shuffled_phase":
                probe = perm[neighbor]
            elif mode == "mirrored_phase":
                probe = base_condition.slot_count - 1 - neighbor
            else:
                probe = neighbor
        ...
        if field_recent:
            ...
            if soft_admit >= soft_threshold:
                ...
                charges += recall_condition.recall_gain * 0.25 * field * (
                    field_charge + incoming_groove + support + recall_condition.phase_bind_gain * phase_bind_strength + window_blend_gain * field_layer + band_support
                )
                charges[slot] += recall_condition.recall_gain * 0.55 * (
                    charges[slot] + incoming_groove + 0.40 * field_layer + support + recall_condition.phase_bind_gain * phase_bind_strength + window_blend_gain * field_layer + band_support - competition
                )
...
def main() -> None:
        ...
        summary_rows.append(
            {
                ...
                "mirrored_phase_gap": real["score"] - next(row for row in subset if row["mode"] == "mirrored_phase")["score"],
    ...
    report = f"""# Golden Zipper v64 - Binding Specificity Nulls
...
- mirrored-phase gap: `{golden['mirrored_phase_gap']:.3f}`
...
"""
    ...
    print(f"golden mirrored-phase gap: {golden['mirrored_phase_gap']:.3f}")
