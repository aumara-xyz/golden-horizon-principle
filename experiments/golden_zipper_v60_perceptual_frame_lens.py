#!/usr/bin/env python3
"""v60 perceptual frame lens.

The core memory field stays fixed. Only the recall lens changes.
"""
...
OUT = ROOT / "golden_zipper_v60_outputs"
V41_SUMMARY = ROOT / "golden_zipper_v41_outputs" / "summary.csv"

SEED = 20260526
RNG = np.random.default_rng(SEED)
MODES = {
    "rigid_readout": {"lens_gain": 0.00, "random_lens": False},
    "touchup_lens": {"lens_gain": 0.05, "random_lens": False},
    "balanced_lens": {"lens_gain": 0.12, "random_lens": False},
    "overwash_lens": {"lens_gain": 0.24, "random_lens": False},
    "random_lens": {"lens_gain": 0.12, "random_lens": True},
}
...
def evaluate_recall(seq: np.ndarray, diag: dict[str, float]) -> dict[str, float]:
    ...
    metrics["lens_shift_rate"] = diag["lens_shift_events"] / max(diag["recall_hits"], 1.0)
    return metrics


def run_sequence(alpha: float, base_condition, recall_condition, lens_gain: float, random_lens: bool):
    ...
    split_events = context_distortions = blocked_contexts = overlap_events = phase_bind_events = macro_window_events = admission_band_events = lens_shift_events = 0
    ...
    for idx in range(base_condition.delay + 2, v41.v31b.v31.LENGTH):
        ...
        if field_recent:
            ...
            lens_field = field.copy()
            lens_field[slot] = 0.0
            if lens_field.sum() > 0:
                lens_field /= lens_field.sum()
                if random_lens:
                    lens_field = lens_field[perm]
            lens_signal = float(np.dot(charges, lens_field))
            lens_support = lens_gain * lens_signal
            if lens_support >= 0.02:
                lens_shift_events += 1

            soft_admit = context_similarity + echo_support + bind_support + band_support + lens_support
            soft_threshold = recall_condition.similarity_threshold + 0.10
            if soft_admit >= soft_threshold:
                if lens_support > 0.04:
                ...
                layer_trace[:, 0] += recall_condition.resonance_gain * (0.6 * field + 0.4 * phase_field) * (0.5 + contact + phase_alignment)
    ...
    diag = {
        ...
        "lens_shift_events": float(lens_shift_events),
    }
    return seq, diag
...
def main() -> None:
    ...
    for flow in sorted({row["flow"] for row in rows}):
        ...
        balanced = next(row for row in subset if row["mode"] == "balanced_lens")
        summary.append(
            {
                "flow": flow,
                ...
                "lens_shift_rate": balanced["lens_shift_rate"],
                "balanced_vs_rigid_gap": balanced["score"] - next(row for row in subset if row["mode"] == "rigid_readout")["score"],
                "balanced_vs_touchup_gap": balanced["score"] - next(row for row in subset if row["mode"] == "touchup_lens")["score"],
                "balanced_vs_overwash_gap": balanced["score"] - next(row for row in subset if row["mode"] == "overwash_lens")["score"],
                "balanced_vs_random_gap": balanced["score"] - next(row for row in subset if row["mode"] == "random_lens")["score"],
            }
        )
    ...
    write_text(OUT / "report.md", f"""# Golden Zipper v60 - Perceptual Frame Lens

Best flow by balanced-lens score: `{best['flow']}` with `{best['balanced_score']:.3f}`
...
- lens shift rate: `{golden['lens_shift_rate']:.3f}`
...
""")
    ...
    print(f"best flow by balanced-lens score: {best['flow']} {best['balanced_score']:.3f}")
