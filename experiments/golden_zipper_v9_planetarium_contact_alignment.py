#!/usr/bin/env python3
"""v9 planetarium contact-alignment toy for Golden Zipper."""
...
OUT = ROOT / "golden_zipper_v9_outputs"
...
def build_models() -> list[ModelSpec]:
    return [
        ModelSpec("single_light_sphere", "Single Light"),
        ModelSpec("two_light_alignment", "Two-Light Alignment"),
        ModelSpec("torus_contact_ring", "Contact Ring"),
        ModelSpec("random_projection_null", "Random Projection Null"),
        ModelSpec("shuffled_time_null", "Shuffled Time Null"),
    ]
...
def build_sequence(model: str, alpha: float, condition: ObserverCondition) -> tuple[np.ndarray, dict[str, np.ndarray | float]]:
    ...
    if model == "single_light_sphere":
    ...
    elif model == "two_light_alignment":
        ...
        near_both = (np.abs(signed1) < 0.58 * condition.window_width) & (np.abs(signed2) < 0.58 * condition.window_width)
        witness = near_both & contact_ring & (alignment < 0.34 * condition.window_width)
...
def evaluate_model_sequence(model: str, alpha: float, condition: ObserverCondition) -> dict[str, float]:
    ...
    composite = float(
        surrogate_core
        + 0.12 * diag["contact_ring_rate"]
        + 0.10 * base["return_interval_diversity"]
        + 0.08 * rigidity
    )
    return {
        **base,
        "contact_ring_rate": float(diag["contact_ring_rate"]),
...
def build_report(anchor_rows: list[dict]) -> str:
    ...
    contact_rows = [row for row in anchor_rows if row["model"] == "torus_contact_ring"]
    top_contact = max(contact_rows, key=lambda row: row["composite_score"])
    golden = next(row for row in contact_rows if row["anchor"] == "golden")
    golden_rank = next(
        idx for idx, row in enumerate(sorted(contact_rows, key=lambda row: row["composite_score"], reverse=True), start=1)
        if row["anchor"] == "golden"
    )
    if (
        model_means["torus_contact_ring"]["composite"] > model_means["single_light_sphere"]["composite"]
        and model_means["torus_contact_ring"]["composite"] > model_means["random_projection_null"]["composite"]
    ):
        verdict = "Promising geometry signal: two-light contact alignment outperforms the simpler recurrence/null models."
    else:
        verdict = "Mixed/negative: contact alignment is interesting, but not yet clearly better than the simpler/null projections."
    return f"""# Golden Zipper v9 - Planetarium Contact Alignment

Toy telemetry only. Not physics evidence. Not proof of GHP. Not proof that phi is fundamental.

## Executive Summary

{verdict}

This run treated memory as delayed alignment of projected lights on an observer sphere.

## Model Means

| Model | Mean composite | Mean hotspot stability | Mean surrogate gap |
|---|---:|---:|---:|
| single_light_sphere | {model_means['single_light_sphere']['composite']:.3f} | {model_means['single_light_sphere']['stability']:.3f} | {model_means['single_light_sphere']['surrogate_gap']:.3f} |
| two_light_alignment | {model_means['two_light_alignment']['composite']:.3f} | {model_means['two_light_alignment']['stability']:.3f} | {model_means['two_light_alignment']['surrogate_gap']:.3f} |
| torus_contact_ring | {model_means['torus_contact_ring']['composite']:.3f} | {model_means['torus_contact_ring']['stability']:.3f} | {model_means['torus_contact_ring']['surrogate_gap']:.3f} |
| random_projection_null | {model_means['random_projection_null']['composite']:.3f} | {model_means['random_projection_null']['stability']:.3f} | {model_means['random_projection_null']['surrogate_gap']:.3f} |
| shuffled_time_null | {model_means['shuffled_time_null']['composite']:.3f} | {model_means['shuffled_time_null']['stability']:.3f} | {model_means['shuffled_time_null']['surrogate_gap']:.3f} |

Best model by mean composite: `{best_model}`

## Contact-Ring Read

- Top contact-ring anchor: `{top_contact['anchor']}` with composite `{top_contact['composite_score']:.3f}`
- Golden contact-ring rank: `{golden_rank}` / `{len(contact_rows)}`
- Golden contact-ring composite: `{golden['composite_score']:.3f}`
- Golden contact-ring stability: `{golden['hotspot_stability']:.3f}`
- Golden contact-ring surrogate gap: `{golden['surrogate_gap']:.3f}`
- Golden mean alignment latency: `{golden['mean_alignment_latency']:.3f}`

## What This Supports

- Delayed alignment on the observer wall is a plausible toy alternative to simple recurrence.
- Contact-ring geometry is worth probing directly.

## What This Does Not Support

- does not prove GHP
- does not prove phi is unique
- does not prove torus/sphere contact is the true observer geometry
- does not justify hardening any paper
"""
...
def main() -> None:
    ...
    print(f"golden contact rank: {sorted(contact_rows, key=lambda row: row['composite_score'], reverse=True).index(golden) + 1}")
    print(f"golden contact composite: {golden['composite_score']:.3f}")
    print(f"golden contact stability: {golden['hotspot_stability']:.3f}")
