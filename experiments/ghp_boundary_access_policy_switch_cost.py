#!/usr/bin/env python3
"""Boundary Access switch-cost sweep for rescue policies.
...
OUT = ROOT / "ghp_boundary_access_policy_switch_cost_outputs"

SWITCH_COSTS = [0.0, 0.01, 0.03, 0.06]
...
def evaluate(policy: Policy, switch_cost: float, words: dict[str, str], vocab_index: dict[str, int]) -> dict[str, float | str]:
    ...
    for _ in range(event.loop.TRIALS):
        ...
        for step in range(event.loop.TIMESTEPS - 1):
            ...
            family_name = context_policy.choose_family(policy.name, damage_mode, helper_mode)
            ...
            switch_penalty = switch_cost if prev_family is not None and family_name != prev_family else 0.0
            prev_family = family_name
            switch_costs_paid.append(switch_penalty)

            masked_self = event.loop.base.cosine(damaged, readable_a)
            repair = max(0.0, event.loop.base.cosine(recovered, readable_a) - switch_penalty)
            repair_scores.append(repair)
            repair_gain_scores.append(max(0.0, repair - masked_self))
            identity_scores.append(max(0.0, event.loop.base.cosine(recovered, deep_trace) - switch_penalty))
            scene_scores.append(max(0.0, event.loop.base.cosine(recovered, readable_a) - switch_penalty))
            direction_scores.append(max(0.0, event.loop.base.cosine(recovered, next_readable) - switch_penalty))
            prev_b = readable_b
    ...
    metrics = {
        ...
        "avg_switch_cost_paid": mean_or_zero(switch_costs_paid),
    }
    metrics["score_switch"] = (
...
def main() -> None:
    ...
    rows = [evaluate(policy, cost, words, vocab_index) for cost in SWITCH_COSTS for policy in policies]
    write_csv(rows, OUT / "switch_cost_metrics.csv")

    lines = [
        "# Boundary Access Policy Switch Cost",
        "",
        f"- switch costs `{SWITCH_COSTS}`",
        "",
        "Best policy by switch cost:",
    ]
    for cost in SWITCH_COSTS:
        subset = [row for row in rows if row["switch_cost"] == cost]
        best = max(subset, key=lambda row: float(row["score_switch"]))
        lines.append(f"- cost `{cost}`: `{best['policy']}` `{float(best['score_switch']):.3f}`")
