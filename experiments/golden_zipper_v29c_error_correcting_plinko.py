#!/usr/bin/env python3
"""v29c error-correcting Plinko boundary.

This version adds the missing perception move from v29b:
small misses nudge the prediction map instead of immediately failing.
...
OUT = ROOT / "golden_zipper_v29c_outputs"
...
CORRECTION_GAINS = [0.04, 0.10, 0.18]
POCKET_GAINS = [0.12, 0.22, 0.34]
POCKET_DECAYS = [0.90, 0.95, 0.985]
FIT_LIMITS = [0.18, 0.30, 0.46]
CORRECTION_LIMITS = [0.34, 0.52, 0.70]
REVISION_THRESHOLDS = [0.012, 0.028, 0.055]
WRITE_THRESHOLDS = [0.34, 0.50, 0.66]
RUPTURE_LIMITS = [0.64, 0.82, 0.96]
MAX_CONDITIONS = 150
...
def build_sequence(alpha: float, condition: Condition) -> tuple[np.ndarray, dict[str, float]]:
    ...
    for idx in range(condition.delay + 2, LENGTH):
        ...
        delta = contact - predicted_now
        revision = condition.prediction_gain * delta
        correction = condition.correction_gain * delta
        ...
        correctable = condition.fit_limit < fit_error <= condition.correction_limit
        ...
        if correctable:
            corrections += 1
            predicted_contact = float(np.clip(predicted_now + correction, 0.0, 1.0))
            contact_trend = (1.0 - condition.correction_gain) * contact_trend + condition.correction_gain * (contact - prev_contact)
            if near_boundary:
                pocket += 0.35 * condition.pocket_gain * contact
                if not pocket_open:
                    seq[idx] = 0
                    witnesses += 1
                    pocket_open = True
        ...
        if rupture:
            ...
            pocket_open = False
            pocket_age = 0
            predicted_contact = 0.985 * predicted_contact + 0.015 * contact
            contact_trend = 0.985 * contact_trend + 0.015 * (contact - prev_contact)
            continue
...
def evaluate(seq: np.ndarray, diag: dict[str, float]) -> dict[str, float]:
    ...
    return {
        ...
        "correction_rate": diag["corrections"] / max(len(seq), 1),
...
def score(metrics: dict[str, float]) -> float:
    ...
    correction_term = min(metrics["correction_rate"] / 0.12, 1.0)
    return float(
        0.16 * metrics["phase_lock_resistance"]
        + 0.18 * metrics["witness_conversion"]
        + 0.14 * metrics["delayed_retention"]
        + 0.12 * metrics["prediction_update_rate"]
        + 0.10 * fit_term
        + 0.09 * revision_term
        + 0.08 * pocket_term
        + 0.07 * correction_term
...
def main() -> None:
        ...
        flow_rows.append(
            {
                ...
                "mean_correction_rate": float(np.mean([row["correction_rate"] for row in subset])),
    ...
    report = f"""# Golden Zipper v29c - Error-Correcting Plinko Boundary
...
- correction rate: `{golden['mean_correction_rate']:.3f}`
...
"""
    ...
    print(f"golden correction rate: {golden['mean_correction_rate']:.3f}")
