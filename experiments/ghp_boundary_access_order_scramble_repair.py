#!/usr/bin/env python3
"""Rank-shape repair test for order-scrambled Boundary Access signals.
...
OUT = ROOT / "ghp_boundary_access_order_scramble_repair_outputs"

TRAIN_SETS = {
    "uniform_only": generalization.TRAIN_SCENARIOS,
    "uniform_plus_permute": generalization.TRAIN_SCENARIOS
    + [
        {"name": "permute_train_030", "helper_kind": "permute_mix", "noise_level": 0.30},
        {"name": "permute_train_045", "helper_kind": "permute_mix", "noise_level": 0.45},
        {"name": "permute_train_060", "helper_kind": "permute_mix", "noise_level": 0.60},
    ],
}
...
def sorted_cosine(a: np.ndarray, b: np.ndarray) -> float:
    return float(local_switcher.base.cosine(np.sort(a), np.sort(b)))
    ...
    try:
        for family_name in local_switcher.FAMILY_NAMES:
            ...
            for scenario in scenarios:
                for variant in generalization.INTERNAL_VARIANTS + generalization.COHERENT_VARIANTS:
                            ...
                            rows.append(
                                {
                                    ...
                                    "variant": variant,
                                    "label": 1 if variant in generalization.COHERENT_VARIANTS else 0,
                                    "target_family": "fibonacci"
                                    if variant in generalization.COHERENT_VARIANTS
...
def rank_shape_features(row: dict[str, object]) -> np.ndarray:
    ...
    base_features = np.asarray(row["base_features"], dtype=float)
    normal_helper_wake = float(base_features[3])
    normal_helper_deep = float(base_features[4])

    sorted_helper_wake = sorted_cosine(helper, wake)
    sorted_helper_deep = sorted_cosine(helper, deep_trace)
    sorted_damage_helper = sorted_cosine(damaged, helper)
    sorted_damage_wake = sorted_cosine(damaged, wake)
    sorted_damage_deep = sorted_cosine(damaged, deep_trace)
    scramble_wake = sorted_helper_wake - normal_helper_wake
    scramble_deep = sorted_helper_deep - normal_helper_deep

    return np.array(
        [
            sorted_helper_wake,
            sorted_helper_deep,
            sorted_damage_helper,
            sorted_damage_wake,
            sorted_damage_deep,
            scramble_wake,
            scramble_deep,
...
def features_for_pack(row: dict[str, object], pack: str) -> np.ndarray:
    ...
    rank_shape = rank_shape_features(row)
    if pack == "baseline_six":
        return baseline
    if pack == "rank_shape_only":
        return rank_shape
    if pack == "helper_rank_only":
        return rank_shape[[0, 1, 5, 6]]
    if pack == "damage_rank_only":
        return rank_shape[[2, 3, 4]]
    if pack == "scramble_signature_only":
        return rank_shape[-2:]
...
def evaluate(
    ...
    summary = {
        ...
        "pack": pack,
        "dimensions": int(len(features_for_pack(test_rows[0], pack))),
...
def main() -> None:
    ...
    try:
        ...
        train_by_set: dict[str, list[dict[str, object]]] = {}
        for name, scenarios in TRAIN_SETS.items():
            rows: list[dict[str, object]] = []
            for seed in local_switcher.TRAIN_SEEDS:
                rows.extend(collect_rows_for_seed(seed, scenarios, words, vocab_index))
            train_by_set[name] = rows
    ...
    scenario_rows: list[dict[str, float | str]] = []
    for train_set_name, train_rows in train_by_set.items():
        for pack in PACKS:
            summary, scenarios = evaluate(train_set_name, train_rows, test_rows, pack)
    ...
    write_csv(summary_rows, OUT / "order_scramble_repair_summary.csv")
    write_csv(scenario_rows, OUT / "order_scramble_repair_scenarios.csv")

    best = summary_rows[0]
    target = [row for row in scenario_rows if row["train_set"] == best["train_set"] and row["pack"] == best["pack"]]
    lines = [
        ...
        f"- best train set: `{best['train_set']}`",
    ...
    for row in summary_rows:
        lines.append(
            "- "
            f"{row['train_set']} / {row['pack']} ({int(row['dimensions'])}D): "
