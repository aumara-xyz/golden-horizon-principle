#!/usr/bin/env python3
"""v34 basin identity recall.

This tests whether a memory lives in a shaped basin rather than a flat local family:
- recall can move inside the same basin and still count as the same memory
- the basin center matters most, but nearby positions still belong
- context can tint the basin without requiring exact-slot return
...
OUT = ROOT / "golden_zipper_v34_outputs"

SEED = 20260523
RNG = np.random.default_rng(SEED)

CONTEXT_PRESSURES = [0.04, 0.08, 0.12]
SIMILARITY_THRESHOLDS = [0.58, 0.68, 0.78]
RECALL_WINDOWS = [13, 21]
RECALL_GAINS = [0.08, 0.14, 0.20]
SHEATH_GAINS = [0.04, 0.08, 0.12]
BLEED_RATES = [0.04, 0.08, 0.12]
SPLIT_THRESHOLDS = [0.24, 0.36]
BASIN_RADII = [1, 2]
BASIN_DEPTHS = [0.35, 0.55]
MAX_VARIANTS = 54
...
def build_recall_conditions() -> list[RecallCondition]:
    full = [
        ...
        for sheath_gain in SHEATH_GAINS
        for bleed_rate in BLEED_RATES
        for split_threshold in SPLIT_THRESHOLDS
        for basin_radius in BASIN_RADII
        for basin_depth in BASIN_DEPTHS
    ]
...
def score_recall(metrics: dict[str, float]) -> float:
    deepen = min(metrics["same_basin_recall_rate"] / 0.52, 1.0)
...
def basin_weights(slot: int, slot_count: int, radius: int, depth: float) -> list[tuple[int, float]]:
    weights: list[tuple[int, float]] = []
    for offset in range(-radius, radius + 1):
        member = (slot + offset) % slot_count
        scale = 1.0 - (abs(offset) / max(radius + 1, 1))
        weight = max(depth + (1.0 - depth) * scale, 0.05)
        weights.append((member, weight))
    return weights


def build_basin_identity_sequence(
    alpha: float,
    base_condition,
    ...
    for idx in range(base_condition.delay + 2, v31b.v31.LENGTH):
        ...
        basin = basin_weights(slot, base_condition.slot_count, recall_condition.basin_radius, recall_condition.basin_depth)
...
def main() -> None:
    ...
    for spec in specs:
        for recall_condition in recall_conditions:
            seq, diag = build_basin_identity_sequence(spec.alpha, spec.condition, recall_condition)
            ...
            direct_rows.append(
                {
                    ...
                    "sheath_gain": recall_condition.sheath_gain,
                    "bleed_rate": recall_condition.bleed_rate,
                    "split_threshold": recall_condition.split_threshold,
                    "basin_radius": recall_condition.basin_radius,
                    "basin_depth": recall_condition.basin_depth,
                    **metrics,
    ...
    for spec in specs:
        ...
        recall_condition = RecallCondition(
            ...
            sheath_gain=best["sheath_gain"],
            bleed_rate=best["bleed_rate"],
            split_threshold=best["split_threshold"],
            basin_radius=int(best["basin_radius"]),
            basin_depth=best["basin_depth"],
        )
        seq, diag = build_basin_identity_sequence(spec.alpha, spec.condition, recall_condition)
        ...
        summary_rows.append(
            {
                ...
                "best_sheath_gain": recall_condition.sheath_gain,
                "best_bleed_rate": recall_condition.bleed_rate,
                "best_split_threshold": recall_condition.split_threshold,
                "best_basin_radius": recall_condition.basin_radius,
                "best_basin_depth": recall_condition.basin_depth,
                ...
                "mean_same_basin_recall_rate": float(np.mean([row["same_basin_recall_rate"] for row in subset])),
    ...
    report = f"""# Golden Zipper v34 - Basin Identity Recall
...
- best sheath gain: `{golden['best_sheath_gain']:.3f}`
- best bleed rate: `{golden['best_bleed_rate']:.3f}`
- best basin radius: `{golden['best_basin_radius']}`
- best basin depth: `{golden['best_basin_depth']:.3f}`
...
- mean same-basin recall rate: `{golden['mean_same_basin_recall_rate']:.3f}`
...
Interpretation:
- This panel tests whether a memory can live in a small basin rather than a flat local family.
- A useful result should raise same-basin recall while allowing exact-slot recall to stay lower.
"""
    ...
    print(f"golden best sheath_gain: {golden['best_sheath_gain']:.3f}")
    print(f"golden best bleed_rate: {golden['best_bleed_rate']:.3f}")
    print(f"golden best basin_radius: {golden['best_basin_radius']}")
    print(f"golden best basin_depth: {golden['best_basin_depth']:.3f}")
    print(f"golden recall survival rate: {golden['mean_recall_survival_rate']:.3f}")
    print(f"golden same-basin recall rate: {golden['mean_same_basin_recall_rate']:.3f}")
