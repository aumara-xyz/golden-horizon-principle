#!/usr/bin/env python3
"""v20 embodied cost panel for model-revision memory."""
...
OUT = ROOT / "golden_zipper_v20_outputs"
...
INSTABILITY_PENALTIES = [0.35, 0.55, 0.75]
MAX_CONDITIONS = 120
...
def build_sequence(alpha: float, condition: Condition, mode: str) -> tuple[np.ndarray, dict[str, float]]:
    ...
    for idx, value in enumerate(intensity):
        ...
        if not near_both[idx]:
            ...
            next_rule = 0.995 * rule_estimate + 0.005 * proposed_rule
            rule_steps.append(abs(next_rule - rule_estimate))
            rule_estimate = next_rule
            continue
        ...
        rule_steps.append(abs(rule_estimate - old_rule))

    rule_steps_arr = np.asarray(rule_steps, dtype=float)
    instability = float(np.std(rule_steps_arr) + np.mean(rule_steps_arr > np.quantile(rule_steps_arr, 0.95)))
    return seq, {
...
def evaluate_model(seq: np.ndarray, diag: dict[str, float], condition: Condition) -> dict[str, float]:
    ...
    embodied_cost = min(condition.instability_penalty * diag["instability"], 1.0)
    return {
...
def main() -> None:
            ...
            for condition in conditions:
                for offset in OFFSET_GRID:
                    ...
                    if not (0.0 < alpha < 1.0):
                        continue
                    seq, diag = build_sequence(alpha, condition, mode)
                    ...
                    direct_rows.append(
                        {
                            ...
                            "instability_penalty": condition.instability_penalty,
                            **metrics,
    ...
    mode_rows: list[dict] = []
    for mode in modes:
        subset = [row for row in direct_rows if row["mode"] == mode]
        mode_rows.append(
            {
                "mode": mode,
                "best_anchor": max(subset, key=lambda row: row["composite_score"])["anchor"],
                "best_composite": float(np.max([row["composite_score"] for row in subset])),
                ...
                "mean_embodied_cost": float(np.mean([row["embodied_cost"] for row in subset])),
                "mean_instability": float(np.mean([row["instability"] for row in subset])),
            }
        )

    ranked = sorted(mode_rows, key=lambda row: row["mean_composite"], reverse=True)
    write_csv(direct_rows, OUT / "direct_metrics.csv")
    write_csv(mode_rows, OUT / "mode_summary.csv")
    plot_mode_summary(mode_rows, OUT / "mode_summary.png")

    real = next(row for row in mode_rows if row["mode"] == "real")
    strongest_null = max((row for row in mode_rows if row["mode"] != "real"), key=lambda row: row["mean_composite"])
    report = f"""# Golden Zipper v20 - Embodied Cost Panel

Toy telemetry only. Not physics evidence. Not proof of GHP.

Best mode by mean composite: `{ranked[0]['mode']}` with `{ranked[0]['mean_composite']:.3f}`

Real-sequence result:
- real mean composite: `{real['mean_composite']:.3f}`
- real embodied cost: `{real['mean_embodied_cost']:.3f}`
- real instability: `{real['mean_instability']:.3f}`

Strongest null:
- `{strongest_null['mode']}` mean composite: `{strongest_null['mean_composite']:.3f}`
- `{strongest_null['mode']}` embodied cost: `{strongest_null['mean_embodied_cost']:.3f}`

Interpretation:
- This panel asks whether model revision should be penalized when it destabilizes the observer.
- A strong result would separate structured real flow from random/phase-scrambled flow by giving real flow lower embodied cost and higher composite.
- This remains toy telemetry only.
"""
    write_text(OUT / "report.md", report)

    print(f"files created: {OUT}")
    print(f"best mode by mean composite: {ranked[0]['mode']}")
    print(f"real mean composite: {real['mean_composite']:.3f}")
    print(f"real embodied cost: {real['mean_embodied_cost']:.3f}")
    print(f"strongest null: {strongest_null['mode']} {strongest_null['mean_composite']:.3f}")
    print(f"strongest null embodied cost: {strongest_null['mean_embodied_cost']:.3f}")
