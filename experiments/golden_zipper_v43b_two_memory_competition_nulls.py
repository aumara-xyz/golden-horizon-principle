#!/usr/bin/env python3
"""v43b two-memory competition nulls.

This attacks the competition-pressure lane:
- balanced rival
- no competition
...
OUT = ROOT / "golden_zipper_v43b_outputs"
V41_SUMMARY = ROOT / "golden_zipper_v41_outputs" / "summary.csv"

MODES = {
    "balanced_rival": (1.00, 1.00),
    "no_competition": (1.00, 0.00),
    "inverted_competition": (1.00, -1.00),
    "flat_competition": (0.80, 0.80),
}
...
def main() -> None:
        ...
        summary_rows.append(
            {
                ...
                "no_competition_gap": balanced["score"] - next(row for row in subset if row["mode"] == "no_competition")["score"],
                "inverted_competition_gap": balanced["score"] - next(row for row in subset if row["mode"] == "inverted_competition")["score"],
                "flat_competition_gap": balanced["score"] - next(row for row in subset if row["mode"] == "flat_competition")["score"],
            }
        )
    ...
    report = f"""# Golden Zipper v43b - Two Memory Competition Nulls
...
- no-competition gap: `{golden['no_competition_gap']:.3f}`
- inverted-competition gap: `{golden['inverted_competition_gap']:.3f}`
- flat-competition gap: `{golden['flat_competition_gap']:.3f}`
"""
    ...
    print(f"golden no-competition gap: {golden['no_competition_gap']:.3f}")
    print(f"golden inverted-competition gap: {golden['inverted_competition_gap']:.3f}")
    print(f"golden flat-competition gap: {golden['flat_competition_gap']:.3f}")
