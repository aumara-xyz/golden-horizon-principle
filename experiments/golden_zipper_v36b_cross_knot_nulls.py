#!/usr/bin/env python3
"""v36b cross-knot nulls.

This attacks the v36 neighbor-interference object:
- real cross-knot interference
- no interference
- shuffled neighbor interaction
- inverted interference sign
...
OUT = ROOT / "golden_zipper_v36b_outputs"
V36_SUMMARY = ROOT / "golden_zipper_v36_outputs" / "summary.csv"

SEED = 20260524
RNG = np.random.default_rng(SEED)
MODES = ["real", "no_interference", "shuffled_neighbor", "inverted_sign"]
    ...
    for idx in range(base_condition.delay + 2, v36.v31b.v31.LENGTH):
        ...
        for neighbor in range(base_condition.slot_count):
            ...
            if recall_condition.basin_radius < distance <= recall_condition.interference_radius:
                ...
                probe = perm[neighbor] if mode == "shuffled_neighbor" else neighbor
                ...
                if mode == "no_interference":
                    signed = 0.0
                elif mode == "inverted_sign":
                    signed = -signed
                if signed >= 0:
                    outer_support += recall_condition.interference_gain * signed * charges[probe] / (1.0 + distance)
                    if signed > 0.25:
                        interference_coherences += 1
                else:
                    outer_competition += recall_condition.competition_gain * abs(signed) * charges[probe] / (1.0 + distance)
...
def main() -> None:
        ...
        summary_rows.append(
            {
                ...
                "no_interference_gap": real["score"] - next(row for row in subset if row["mode"] == "no_interference")["score"],
                "shuffled_neighbor_gap": real["score"] - next(row for row in subset if row["mode"] == "shuffled_neighbor")["score"],
                "inverted_sign_gap": real["score"] - next(row for row in subset if row["mode"] == "inverted_sign")["score"],
            }
        )
    ...
    report = f"""# Golden Zipper v36b - Cross-Knot Nulls
...
- no-interference gap: `{golden['no_interference_gap']:.3f}`
- shuffled-neighbor gap: `{golden['shuffled_neighbor_gap']:.3f}`
- inverted-sign gap: `{golden['inverted_sign_gap']:.3f}`

Interpretation:
- This panel attacks whether neighboring knot-family interactions are actually doing work.
- A useful result keeps positive gaps when interference is removed, scrambled, or sign-inverted.
"""
    ...
    print(f"golden no-interference gap: {golden['no_interference_gap']:.3f}")
    print(f"golden shuffled-neighbor gap: {golden['shuffled_neighbor_gap']:.3f}")
    print(f"golden inverted-sign gap: {golden['inverted_sign_gap']:.3f}")
