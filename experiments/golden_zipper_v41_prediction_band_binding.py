...
This adds a soft prediction/admission band on top of the stronger field model:
- relational field identity
- phase binding inside the field
- nested observer windows
- moderate mismatch is admitted better than perfect lock or wild rupture
...
OUT = ROOT / "golden_zipper_v41_outputs"
...
PHASE_BIND_GAINS = [0.08, 0.14]
PHASE_BIND_THRESHOLDS = [0.18, 0.30, 0.42]
WINDOW_LEVELS = [1, 2, 3]
WINDOW_BLEND_GAINS = [0.08, 0.14]
ADMISSION_CENTERS = [0.10, 0.20, 0.30]
ADMISSION_WIDTHS = [0.10, 0.18]
ADMISSION_GAINS = [0.08, 0.14]
...
class RecallCondition:
    ...
    phase_bind_gain: float
    phase_bind_threshold: float
    window_levels: int
    window_blend_gain: float
    admission_center: float
    admission_width: float
    admission_gain: float
...
def build_recall_conditions() -> list[RecallCondition]:
    full = [
        RecallCondition(
            ...
            phase_bind_gain=phase_bind_gain,
            phase_bind_threshold=phase_bind_threshold,
            window_levels=window_levels,
            window_blend_gain=window_blend_gain,
            admission_center=admission_center,
        ...
        for phase_bind_gain in PHASE_BIND_GAINS
        for phase_bind_threshold in PHASE_BIND_THRESHOLDS
        for window_levels in WINDOW_LEVELS
        for window_blend_gain in WINDOW_BLEND_GAINS
        for admission_center in ADMISSION_CENTERS
        for admission_width in ADMISSION_WIDTHS
        for admission_gain in ADMISSION_GAINS
...
def score_recall(metrics: dict[str, float]) -> float:
    ...
    binding = min(metrics["phase_binding_rate"] / 0.45, 1.0)
    macro = min(metrics["macro_window_rate"] / 0.45, 1.0)
    band = min(metrics["admission_band_rate"] / 0.35, 1.0)
    return float(
...
def build_field_sequence(
    ...
    split_events = context_distortions = blocked_contexts = overlap_events = phase_bind_events = macro_window_events = admission_band_events = 0
    ...
    for idx in range(base_condition.delay + 2, v31b.v31.LENGTH):
        ...
        for level in range(2, recall_condition.window_levels + 1):
            coarse_kernel = temporal_kernel(
        ...
        for neighbor in range(base_condition.slot_count):
            if field[neighbor] == 0.0:
                continue
            neighbor_phase = ((neighbor * alpha) + idx / max(base_condition.delay, 1)) % 1.0
        ...
        if field_recent:
            ...
            band_shape = np.exp(
                -0.5
                * ((prediction_error - recall_condition.admission_center) / max(recall_condition.admission_width, 1e-9))
                ** 2
            )
            band_support = recall_condition.admission_gain * float(band_shape)
            ...
            if soft_admit >= soft_threshold:
                ...
                charges += recall_condition.recall_gain * 0.25 * field * (
                    ...
                    + recall_condition.window_blend_gain * field_layer
                    + band_support
                )
...
def evaluate_recall(seq: np.ndarray, diag: dict[str, float]) -> dict[str, float]:
    ...
    return {
        ...
        "phase_binding_rate": diag["phase_bind_events"] / max(len(seq), 1.0),
        "macro_window_rate": diag["macro_window_events"] / max(len(seq), 1.0),
        "admission_band_rate": diag["admission_band_events"] / max(len(seq), 1.0),
...
def main() -> None:
            ...
            direct_rows.append(
                {
                    ...
                    "phase_bind_gain": recall_condition.phase_bind_gain,
                    "phase_bind_threshold": recall_condition.phase_bind_threshold,
                    "window_levels": recall_condition.window_levels,
                    "window_blend_gain": recall_condition.window_blend_gain,
                    "admission_center": recall_condition.admission_center,
                    "admission_width": recall_condition.admission_width,
    ...
    for spec in specs:
        ...
        seq, diag = build_field_sequence(
            ...
            RecallCondition(
                ...
                phase_bind_gain=best["phase_bind_gain"],
                phase_bind_threshold=best["phase_bind_threshold"],
                window_levels=int(best["window_levels"]),
                window_blend_gain=best["window_blend_gain"],
                admission_center=best["admission_center"],
                admission_width=best["admission_width"],
                admission_gain=best["admission_gain"],
        ...
        density_diag = {
            ...
            "overlap_events": 0.0,
            "phase_bind_events": 0.0,
            "macro_window_events": 0.0,
            "admission_band_events": 0.0,
        }
        summary_rows.append(
            {
                "flow": spec.flow,
                ...
                "best_phase_bind_gain": best["phase_bind_gain"],
                "best_phase_bind_threshold": best["phase_bind_threshold"],
                "best_window_levels": best["window_levels"],
                "best_window_blend_gain": best["window_blend_gain"],
                "best_admission_center": best["admission_center"],
                "best_admission_width": best["admission_width"],
                "best_admission_gain": best["admission_gain"],
                ...
                "mean_phase_binding_rate": float(np.mean([row["phase_binding_rate"] for row in subset])),
                "mean_macro_window_rate": float(np.mean([row["macro_window_rate"] for row in subset])),
                "mean_admission_band_rate": float(np.mean([row["admission_band_rate"] for row in subset])),
    ...
    report = f"""# Golden Zipper v41 - Prediction-Band Binding
...
- best phase bind gain: `{golden['best_phase_bind_gain']:.3f}`
- best phase bind threshold: `{golden['best_phase_bind_threshold']:.3f}`
- best window levels: `{golden['best_window_levels']}`
- best window blend gain: `{golden['best_window_blend_gain']:.3f}`
- best admission center: `{golden['best_admission_center']:.3f}`
- best admission width: `{golden['best_admission_width']:.3f}`
- best admission gain: `{golden['best_admission_gain']:.3f}`
...
- mean phase-binding rate: `{golden['mean_phase_binding_rate']:.3f}`
- mean macro-window rate: `{golden['mean_macro_window_rate']:.3f}`
- mean admission-band rate: `{golden['mean_admission_band_rate']:.3f}`
...
Interpretation:
- This panel tests whether multiscale observer windows and phase binding work better together than either alone.
- A useful result should improve field-level identity while keeping split pressure low.
"""
    ...
    print(f"golden best phase_bind_gain: {golden['best_phase_bind_gain']:.3f}")
    print(f"golden best phase_bind_threshold: {golden['best_phase_bind_threshold']:.3f}")
    print(f"golden best window_levels: {golden['best_window_levels']}")
    print(f"golden best window_blend_gain: {golden['best_window_blend_gain']:.3f}")
    print(f"golden best admission_center: {golden['best_admission_center']:.3f}")
    print(f"golden best admission_width: {golden['best_admission_width']:.3f}")
    print(f"golden best admission_gain: {golden['best_admission_gain']:.3f}")
    ...
    print(f"golden phase-binding rate: {golden['mean_phase_binding_rate']:.3f}")
    print(f"golden macro-window rate: {golden['mean_macro_window_rate']:.3f}")
    print(f"golden admission-band rate: {golden['mean_admission_band_rate']:.3f}")
