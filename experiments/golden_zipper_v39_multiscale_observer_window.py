...
This tests whether observer windows should be multiscale:
- the observer can integrate at local and macro widths
- broader windows can blur micro-events into stable mosaics
- memory may survive better when recall can choose among nested present windows
...
OUT = ROOT / "golden_zipper_v39_outputs"
...
def build_field_sequence(
    ...
    split_events = context_distortions = blocked_contexts = overlap_events = macro_window_events = 0
    ...
    for idx in range(base_condition.delay + 2, v31b.v31.LENGTH):
        ...
        for level in range(2, recall_condition.window_levels + 1):
            coarse_kernel = temporal_kernel(recall_condition.layer_window, min(0.92, recall_condition.recency_decay ** (1 / level)))
        ...
        if field_recent:
            ...
            if soft_admit >= soft_threshold:
                ...
                charges += recall_condition.recall_gain * 0.25 * field * (
                    field_charge + incoming_groove + support + recall_condition.window_blend_gain * field_layer
                )
...
def main() -> None:
    ...
    report = f"""# Golden Zipper v39 - Multiscale Observer Window
...
Interpretation:
- This panel tests whether nested observer windows stabilize recall.
- A useful result should improve field-level survival without forcing exact-core pinning.
"""
