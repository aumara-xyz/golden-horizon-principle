#!/usr/bin/env python3
"""v24 stabilized pocket render loop.

Toy telemetry only. Not physics evidence. Not proof of GHP.

This panel directly tests the user's correction:
- baseline: accumulate immediately on contact
- candidate: stabilize first, then accumulate render-weight
"""
...
OUT = ROOT / "golden_zipper_v24_outputs"
...
STABILITY_GAINS = [0.10, 0.18, 0.28]
STABILITY_THRESHOLDS = [0.20, 0.35, 0.50]
...
def build_sequence(model: str, alpha: float, condition: Condition) -> tuple[np.ndarray, dict[str, float]]:
    ...
    for idx in range(1, LENGTH):
        ...
        if near_boundary:
            if model == "accumulate_immediate":
                pocket += condition.pocket_gain * contact_quality
            else:
                stability += condition.stability_gain * coherence
                if stability >= condition.stability_threshold:
                    pocket += condition.pocket_gain * contact_quality
        ...
        if witness_open and pocket >= condition.write_threshold and near_boundary:
            seq[idx] = 1
            writes += 1
            witness_to_write += 1
            if model == "stabilize_then_accumulate" and stability >= condition.stability_threshold:
                stable_before_write += 1
            pocket *= 0.35
            witness_open = False
            continue
    ...
    return seq, {
        ...
        "stable_before_write": float(stable_before_write),
...
def evaluate_model(seq: np.ndarray, diag: dict[str, float]) -> dict[str, float]:
    ...
    stability_success = diag["stable_before_write"] / max(writes, 1.0)
...
def plot_model_summary(rows: list[dict], path: Path) -> None:
    labels = [f"{row['model']}:{row['flow']}" for row in rows]
...
def main() -> None:
    ...
    models = ["accumulate_immediate", "stabilize_then_accumulate"]
                    ...
                    direct_rows.append(
                        {
                            ...
                            "stability_threshold": condition.stability_threshold,
    ...
    for model in models:
        for flow in flows:
            subset = [row for row in direct_rows if row["model"] == model and row["flow"] == flow.name]
            ...
            seq, _ = build_sequence(model, best["alpha"], cond)
    ...
    plot_model_summary(summary_rows, OUT / "summary.png")

    immediate_best = max((row for row in summary_rows if row["model"] == "accumulate_immediate"), key=lambda row: row["mean_composite"])
    stable_best = max((row for row in summary_rows if row["model"] == "stabilize_then_accumulate"), key=lambda row: row["mean_composite"])
    stable_golden = next(row for row in summary_rows if row["model"] == "stabilize_then_accumulate" and row["flow"] == "golden")

    report = f"""# Golden Zipper v24 - Stabilized Pocket Render Loop

Toy telemetry only. Not physics evidence. Not proof of GHP.

Best immediate-accumulation flow: `{immediate_best['flow']}` with `{immediate_best['mean_composite']:.3f}`
Best stabilize-then-accumulate flow: `{stable_best['flow']}` with `{stable_best['mean_composite']:.3f}`

Golden stabilized result:
- mean composite: `{stable_golden['mean_composite']:.3f}`
- witness conversion: `{stable_golden['mean_witness_conversion']:.3f}`
- stability success: `{stable_golden['mean_stability_success']:.3f}`
- density gap: `{stable_golden['mean_density_gap']:.3f}`

Interpretation:
- This panel directly tests whether memory writes improve when the pocket stabilizes before it starts accumulating render-weight.
"""
    write_text(OUT / "report.md", report)

    print(f"files created: {OUT}")
    print(f"best immediate flow: {immediate_best['flow']} {immediate_best['mean_composite']:.3f}")
    print(f"best stabilized flow: {stable_best['flow']} {stable_best['mean_composite']:.3f}")
    print(f"golden stabilized composite: {stable_golden['mean_composite']:.3f}")
    print(f"golden stabilized witness conversion: {stable_golden['mean_witness_conversion']:.3f}")
    print(f"golden stabilized stability success: {stable_golden['mean_stability_success']:.3f}")
    print(f"golden stabilized density gap: {stable_golden['mean_density_gap']:.3f}")
