#!/usr/bin/env python3
"""v32b contextual recall pressure.

This extends v32 by varying the present context at recall time:
- calm recall should deepen the same groove
- altered context can bend the groove
- high pressure can distort or split the memory family
...
OUT = ROOT / "golden_zipper_v32b_outputs"

SEED = 20260523
RNG = np.random.default_rng(SEED)

CONTEXT_PRESSURES = [0.0, 0.10, 0.22, 0.36]
...
def score_recall(metrics: dict[str, float]) -> float:
    ...
    distortion = 1.0 - min(metrics["context_distortion_rate"] / 0.45, 1.0)
    return float(
...
def build_contextual_recall_sequence(
    alpha: float,
    base_condition,
    ...
    for idx in range(base_condition.delay + 2, v31b.v31.LENGTH):
        ...
        if idx - memory_age[slot] <= recall_condition.recall_window:
            ...
            if target_slot == slot:
                ...
                charges[slot] += recall_condition.recall_gain * (1.0 - recall_condition.context_pressure) * 0.5
...
def plot_summary(rows: list[dict], path: Path) -> None:
    ...
    distortion = [row["mean_context_distortion_rate"] for row in rows]
    x = np.arange(len(labels))
    width = 0.25
    plt.figure(figsize=(11, 5))
    plt.bar(x - width, score, width=width, label="mean score")
    plt.bar(x, survival, width=width, label="recall survival")
    plt.bar(x + width, distortion, width=width, label="context distortion")
...
def main() -> None:
    ...
    for spec in specs:
        for recall_condition in recall_conditions:
            seq, diag = build_contextual_recall_sequence(spec.alpha, spec.condition, recall_condition)
    ...
    report = f"""# Golden Zipper v32b - Contextual Recall Pressure
...
Interpretation:
- This panel asks how much present context bends a remembered groove.
- A useful result would show high same-slot survival under low pressure and controlled distortion under higher pressure.
"""
    ...
    print(f"golden context distortion rate: {golden['mean_context_distortion_rate']:.3f}")
