#!/usr/bin/env python3
"""v22b soft zero-boundary membrane panel.

Toy telemetry only. Not physics evidence. Not proof of GHP.

This version treats zero as a soft observer membrane rather than a hard gate.
Hidden flow can accumulate boundary tension before becoming witness, write,
release, or overload.
"""
...
OUT = ROOT / "golden_zipper_v22b_outputs"
...
WRITE_THRESHOLDS = [0.38, 0.52, 0.68]
OVERLOAD_THRESHOLDS = [1.15, 1.40, 1.75]
...
def build_sequence(alpha: float, condition: Condition) -> tuple[np.ndarray, dict[str, float]]:
    ...
    for idx in range(1, LENGTH):
        ...
        if membrane >= condition.overload_threshold:
            seq[idx] = 2
            overload_total += 1
            membrane = 0.0
            witness_open = False
        ...
        if membrane >= condition.write_threshold and local_stability <= condition.window_width:
            seq[idx] = 1
            stable_crossings += 1
...
def composite_score(metrics: dict[str, float]) -> float:
    crossing_quality = min(max((metrics["mean_crossing_quality"] + 0.3) / 1.2, 0.0), 1.0)
    return float(
...
def main() -> None:
    ...
    report = f"""# Golden Zipper v22b - Soft Zero Boundary
...
Interpretation:
- This panel treats zero as a soft membrane. Hidden flow can build tension before becoming witness, write, release, or overload.
- Success here means the zero-boundary idea works better as a membrane than as a hard gate.
"""
