...
Fable: L3 ordering self-correction (registered order actually held); L1 "exactly 200 dips" (202); smooth-count sup 1.01 (1.234); comb 571 (572); λ₁₀₀ tail power; Maass parity labels reversed; "Selberg control shows no closeness" (identity); Lehmer pair top-5 (6th); KS<0.3 and pair-corr<0.02 missed by 0.002/0.003; KS falling monotonically above 10¹² (noise floor); valley fraction ≈½ (measured ⅓); blanket "γ² horizon for every criterion" (ill-posed). Added after diffing with Codex's summary: toy-height KS order A>B>C≥D (B beat C and D); toy T2 period-shift estimate 1.5 measured 2.05 with no tolerance stated; L3 periodic comb predicted exact, measured paired by centered-difference dispersion.
