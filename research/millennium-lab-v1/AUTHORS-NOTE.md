...
(b) The angle itself is small and decreasing: sin θ(λ) = ‖ξ_λ − c k_λ‖ (unit vectors, optimal c) is 4.1e-4 at x=5, 6.8e-5 at x=13, 3.0e-5 at x=20 (N=144), ≈1e-5 at x=30–36 (N=240–280; at these x the value is resolved only to a factor ≈2 under truncation changes). The threshold λ^{−½}(ln λ)^{−½} at x=36 is ≈0.3. Numerically (★) holds by four orders of magnitude at every tested window; its RATE is not resolved beyond "steeper than needed" (fits between −1 and −4 in x depending on truncation).
