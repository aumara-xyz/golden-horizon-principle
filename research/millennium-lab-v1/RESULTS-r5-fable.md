...
## Reconciliation after Codex round 5b (added later)
- Q1 MEASURED at x=13 and 14: the prime-free (prolate/dilation-identity) candidate reaches 2.9e-30 and 4.0e-33 on the first zero; the true Weil ground state reaches 2.4e-55 and 1.1e-60. Gap 25 and 27.5 orders of magnitude, above the frozen 20-order threshold. The finite Euler product carries arithmetic beyond the identity. At x=9 the gap is 14.9 orders: UNVERIFIED by the frozen rule. The per-k ratio shrinks with k (7e14 at k=1 to 65 at k=20 for x=9), so the primes' extra precision is concentrated on the low zeros, exactly where the RMSE-on-20–50 rule could not see it. My earlier scoring rule is confirmed to have been the wrong instrument.
- Q2 MEASURED dead as posed: the affine-Galerkin commutator residual exceeds the ground-state gap by 35–72 orders at every x, and the controls exceed their own gaps too, so no commuting-operator trigger. Codex's own caveat stands: PW_λ and QW_λ^N do not act on the same space natively, so this rules out one specific bridge, not every prolate bridge.
- Two implementations agreed to ≥31 digits on every displayed entry. Lab conclusion for the bell: bell, not mirror, at x=13 and 14; and the mirror alone already rings to 30 digits, which is the identity Riemann found in 1859.
