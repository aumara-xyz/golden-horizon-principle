...
|---|---|---|
| D30 DeepSeek: certified visible-form floor table (five floors per parity, 0.4-0.7 at T=160 plus L=0.7 at T=240; all new floors x1.18-1.23 over D22, L=0.7 x1.000 because its visible set is the full set); D23 e^{2L} profile SURVIVES on certified floors (rel RMS 3.4% even / 9.1% odd); three edge brackets at T=1024: L=0.5 even CLOSES (1.0947), L=0.6 odd OPEN (1.2869) and L=0.6 even OPEN (1.3383) with the deciding stage the compact term (W_lo/ell = 1.221 before any tail) — the method's edge located at L=0.6 as an optimality-gap jump, not a tail failure | [D30 results](../experiments/deepseek_d30_edge/RESULTS.md) | [Ledger](../experiments/deepseek_d30_edge/PREDICTIONS.md) |
| D28 DeepSeek: court-judged construction-rule loop (frozen court, models edit only construct_candidate.py): **both open cases closed at T=1024** — L=0.5 odd gate ratio 1.0801, L=0.4 even 1.0590 (both < 1.10; L=0.4 odd unchanged at 1.0884); baseline optimizer found the winning rule with no models; loop keeps at iters 2 (deepseek) and 8 (qwen), $0.135 total; controls: wrong-L threading failure, floor-only enclosure 2.28% vs 0.75%, gaming proposer tombstoned by guard | [D28 results](../experiments/deepseek_d28_construction_loop/RESULTS.md) | [Ledger](../experiments/deepseek_d28_construction_loop/PREDICTIONS.md) |
...
| D29 Fable: cross-machine (arm64 Mac) replay of the three closed T = 1024 brackets — digit-for-digit; gate must be W_hi/ℓ, still closes (1.0884, 1.0882, 1.0643) | [Fable D29 replay](../experiments/fable_d29_replay/RESULTS.md) | [logs](../experiments/fable_d29_replay/) |
| D30 Fable (Mac, zero cost): complete certified visible-form floor table, both parities, L = 0.4–0.7 (+14–27 % over D22 where shifts were invisible); certified slopes 53/65/85 (even), 44/59/80 (odd); e^{2L} law fits ≤ 9 %, linear fails | [Fable D30 results](../experiments/fable_d30_floor_table/RESULTS.md) | [Predictions](../experiments/fable_d30_floor_table/PREDICTIONS.md), [certificates](../experiments/fable_d30_floor_table/) |
| D33 Fable (Mac): L = 0.6 floors at T = 240 rise 9 % (both parities), brackets 1.25/1.26 → 1.14/1.15 with frozen D32 winners, still open; T = 320 pending at 320 bits (192-bit run failed: Bessel precision trap recorded) | [Fable D33 results](../experiments/fable_d33_floor_T/RESULTS.md) | [Predictions](../experiments/fable_d33_floor_T/PREDICTIONS.md), [certificates](../experiments/fable_d33_floor_T/) |
