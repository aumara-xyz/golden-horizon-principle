...
- `GHP-PACKET-20260802-01` — Night-close packet. SILVER-OPT-SCALE: **S2_NON_MONOTONE** — the tear line reproduces at its home size (n=256: Δ_sg = +0.287 [+0.278, +0.295]), decays through 320/384 (+0.084/+0.082, CI-solid), flips NEGATIVE at 448 (−0.033, CI excludes 0), goes null at 512/640/768, and RE-EMERGES CI-solid positive at 1024 (+0.0088 [+0.0033, +0.0140]). Not a simple finite-size die-off; per the signed contract: shape reported, no story; follow-ups need new contracts. SYK-CORRIDOR: laptop validation RUN (build + GUE sanity PASS); **NEBIUS BLOCKED BY THE MACHINE ITSELF, ZERO SPEND** — three contract-grounded blockers: (1) the signed d-convention ("d ~ 2^N", glossary line 2072) conflicts with master §5.7's "d = Hilbert space dimension" (= 2^(N/2) for Majorana SYK), and under the pinned reading the GENERIC GUE standard answer computes to β ≈ 0.9–1.0 — INSIDE pass band B1 — failing the discriminator criterion ("a test that cannot fail cannot inform"); (2) the pinned κ grid fails the bracketing rule under the reconstructable normalization (laptop κ_c = 17.95 with one point above → mechanically VOID; a paid rerun reproduces the void); (3) Γ operationalization not source-pinned (IC-3/IC-4 disclosed, not certified). Auth verified working (whoami ACTIVE); the block is contract law, not infrastructure. OWNER DECISIONS REQUIRED before any SYK spend: (a) resolve the d convention by signed erratum or new prereg; (b) re-derive the κ grid under the resolved normalization; (c) pin the Γ operationalization in writing. Evidence: `experiments/syk_corridor/LAPTOP_REPORT.md`, `experiments/silver_opt_scale_results/`.
